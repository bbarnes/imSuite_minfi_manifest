...
# ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
#
#                     Pre-processing:: EPICv1/v2/MSA
#
# ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #

# sesameData::sesameDataList() %>% as.data.frame() %>% tibble::as_tibble() %>% print(n=10000) 

mm10_mft = sesameDataGet("MM285.address")$ordering
epic_mft = sesameDataGet("EPIC.address")$ordering
# epic_mft %>% tibble::as_tibble() %>% dplyr::mutate( Probe_Type = Probe_ID %>% stringr::str_sub(1,2) ) %>% dplyr::group_by( Probe_Type ) %>% dplyr::summarise( Count=n(), .groups = "drop" )
    ...
    if ( !opt$run_sesame && 
      ...
      for ( rm_outliers in rm_outliers_vec ) {
        ...
        for ( exp_name in names(sel_ssh_list) ) {
          ...
          cur_out_path <- NULL
          cur_out_path <- safe_mkdir( dir = file.path(opt$out_path, "sdf",outliers_str, paste0(exp_name,"_",outliers_str) ) )
          ...
          cur_dB_min_all_csv <- file.path( 
            cur_out_path, paste0( 
              opt$run_name,"_",work_step,"_min-pval-",opt$min_pval,"_",exp_name,"_",outliers_str,"","_dB_min_all.csv.gz" ) )
          ...
          cur_dB_max_all_csv <- NULL
          cur_dB_max_all_csv <- file.path( 
            cur_out_path, paste0( 
              opt$run_name,"_",work_step,"_min-pval-1.0","_",exp_name,"_",outliers_str,"","_dB_max_all.csv.gz" ) )
          readr::write_csv( x = cur_dB_max_all_tib, file = cur_dB_max_all_csv )
