...
#
# Toy Summary Example with one sample::
# [TBD]: Parse over experiements and apply this in a graph
#
if ( FALSE ) {
  dplyr::bind_rows(
    dplyr::filter( pre_sdf_list[[1]][[1]], Probe_ID %>% stringr::str_starts("ctl_Negative_") ) %>% dplyr::mutate( Outliers = "rm" ),
    dplyr::filter( pre_sdf_list[[2]][[1]], Probe_ID %>% stringr::str_starts("ctl_Negative_") ) %>% dplyr::mutate( Outliers = "ok" ),
    NULL
  ) %>%
    dplyr::select( Outliers,UG,UR, mask_0,mask_1 ) %>%
    tidyr::pivot_longer( cols = c(mask_0,mask_1), names_to = "mask_grp", values_to = "mask_val" ) %>%
    # dplyr::group_by( Outliers,mask_0 ) %>% 
    # dplyr::group_by( Outliers,mask_1 ) %>% 
...
uhm_train_ssh_tib <- train_sentrix_tib %>%
  dplyr::filter( Sample_GroupKey == "T" )
...
for ( outliers_val in outliers_vec ) {
  ...
  for ( mask_val in mask_vec ) {
    ...
    for ( exp_key in names(rep_train_ssh_tibs) ) {
      ...
      cur_ssh_tib <- NULL
      cur_ssh_tib <- rep_train_ssh_tibs[[exp_key]]
      ...
      #
      for ( sentrix_name in cur_ssh_tib$Sentrix_Name ) {
        pre_sdf_tib <- NULL
        ...
        ses_sdf <- NULL
        ses_sdf <- format_rcpp_sdf( sdf_tib = pre_sdf_tib, 
                                        stat_idx = mask_val, 
                                        cgn_pval = 0.05, 
                                        add_cgns = TRUE, add_negs = TRUE ) %>%
    ...
    #
    # Loop over experiments:: Titration
    #
