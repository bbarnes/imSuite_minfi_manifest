                         ...
                         outDir, write_out=FALSE, plot_heat=FALSE ) 
{
  ...
  # [TBD]: Remove tmp code above.
  # [TBD]: if ssh_tib is NULL or can't find nstr then default to Sentrix_Name
  #  NOTE: we're doing it this way to preserve previous VA:MVP:EPICv2 code...
  if ( is.null(ssh_tib) ) nstr <- "Sentrix_Name"
  ...
  snv_dat <- vcfs %>% # head(n=3) %>% 
    ...
    lapply( parse_snv_vcf, gts_min = gts_min, fval = fval, 
            jcnt = jcnt, uval = uval ) %>%
    stack_snv_list( ssh = ssh_tib, sub = sub_vec ) %>%
    split_snv_stack( nstr = nstr, istr = istr )
