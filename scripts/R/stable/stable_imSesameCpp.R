...
ssh_uhm0_tib <- ssh_uhm0_dat %>%
  ...
  dplyr::mutate( 
    ...
    Sample_Fix = Sample_Name %>% 
      ...
      stringr::str_remove_all( " " ) %>%
      stringr::str_remove( "ng$" ),
    Sample_Titration = Sample_Name %>% 
      stringr::str_remove("^.*-") %>% 
      stringr::str_remove(" ng$") %>% 
      stringr::str_remove( "^.* " ) %>% as.integer(),
    Sample_Input = Sample_Plate %>%
      stringr::str_remove_all( " " ) %>%
      stringr::str_remove( "%" ) %>%
      stringr::str_remove( "ng$" ),
    Sample_Input = dplyr::case_when(
      is.na(Sample_Input) ~ "0",
      TRUE ~ Sample_Input ) %>% as.integer(),
    User_Format = NA_character_,
    Source_Key = "T0"
  ) %>%
  dplyr::select( Source_Key,Source_Name,Sentrix_Name,
...
ssh_uhm1_tib <- ssh_uhm1_dat %>%
  ...
  dplyr::mutate( 
    Sample_Base = dplyr::case_when(
      ...
      TRUE ~ Sample ) %>% stringr::str_to_upper(),
...
# neg_ctl_tib %>% dplyr::filter( Probe_ID %>% stringr::str_detect("_neg_") )
