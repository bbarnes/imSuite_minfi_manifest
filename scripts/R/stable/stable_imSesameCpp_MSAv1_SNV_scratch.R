...
par$prgm_dir <- 'stable'
par$prgm_tag <- 'stable_imSesameCpp_MSAv1_SNV'
par$verbose  <- 3
local_paths  <- c( 
...
if ( FALSE ) {
  ...
  # A tibble: 301,805 × 10
  msa_man_tib <- NULL
  msa_man_tib <- build_core_msa_man( neg_rds = neg_ctl_rds, 
                                     m03_csv = v03_man_csv,
                                     m10_csv = v10_man_csv,
                                     out_csv = msa_man_csv,
                                     vb=vb,vt=vt+3,tc=tc )
  ...
  # Investigation Quick Code::
  #
  # msa_all_ctls_tib %>% print_sum( vec = c("Probe_Type") ) %>% print(n=1000)
  # ses_ctl_tib %>% print_sum( vec = c("Type") ) %>% print(n=1000)
  # msa_all_ctls_tib %>% dplyr::filter( Probe_Type %in% ses_ctl_tib$Type ) %>% print_sum( vec=c("Probe_Type") ) %>% print(n=1000)
  ...
  chip_key <- "207675480004_R09C01"
  sent_vec <- NULL
  sent_vec <- c(chip_key) # ,"207675480004_R09C02")
  ...
  for ( rm_outliers in rm_outliers_vec ) {
    ...
    pre_sdf_path <- safe_mkdir( file.path( opt$top_path, "tmp", "pre_sdf",outliers_str ) )
    
    pre_sdf_list[[outliers_str]] <- NULL
    pre_sdf_list[[outliers_str]] <- sentrix_list %>% # sentrix_list[sent_vec] %>%
      # sentrix_list %>% # head(n=2) %>%
    ...
    # pre_sdf_list[[outliers_str]][[1]] %>% dplyr::filter( Probe_Type == "Ne" )
    # pre_sdf_list[[outliers_str]][[1]] %>% print_sum( vec=c("Probe_Type") )
    
    #
    # [TBD]: This is where we loop around { ctl(min_pval,neg_pval), min_pval,work_vec }
    # [TBD]: Rename min_pval => ctl_pval
    # [TBD]: Loop over masking pval thresholds/validate it makes a difference
    #
    ...
    #
    # Build Sesame Controls::
    #
    ses_ctl_tib <- NULL
    ses_ctl_tib <- pre_sdf_list[[outliers_str]][[chip_key]] %>% 
      sdf_to_sesame_ctls( 
        # ctl_tib = msa_man_tib, 
        ctl_tib = msa_all_ctls_tib, 
        sentrix_name = chip_key, 
        outliers_str = outliers_str,
        ctl_pval = ctl_pval, 
        neg_pval = neg_pval, 
        stat_vec = c(0,1),
        out_path = pre_sdf_path,
        ret_data = FALSE )
    
    # FIX THIS:: msa_all_ctls_tib[Annotation]
    
    # ses_ctl_list[[outliers_str]] <- NULL
    # ses_ctl_list[[outliers_str]] <- ses_ctl_tib
    
    ses_ctl_tib1 <- NULL
    ses_ctl_tib1 <- pre_sdf_list[[outliers_str]][[chip_key]] %>%
      dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ct") ) %>%
      dplyr::mutate(
        col = as.factor(col),
        mask = TRUE
        # mask = dplyr::case_when( Pval_0 > cgn_pval ~ TRUE, TRUE ~ FALSE )
      ) %>%
      dplyr::filter( !(stringr::str_starts( Probe_ID, pattern = "ctl_Negative") &
                       UG_Det_1 < neg_pval & UR_Det_1 < neg_pval) ) %>%
      dplyr::select( Probe_ID:mask ) %>%
      dplyr::arrange( Probe_ID ) %>%
      dplyr::distinct( Probe_ID, .keep_all = TRUE ) %>%
      # dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>%
      dplyr::select( Probe_ID, MG,MR,UG,UR,col, mask )
    
    # ses_ctl_tib0 %>% dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>% head()

    #
    # Build Sesame Body::
    #
    ses_cgn_tib <- NULL
    ses_cgn_tib <- pre_sdf_list[[outliers_str]][[chip_key]] %>%
      dplyr::filter( !stringr::str_starts( Probe_ID, pattern = "ct") ) %>%
      dplyr::mutate(
        col = as.factor(col),
        mask = dplyr::case_when(
          # Pval_0 > cgn_pval ~ TRUE,
          Pval_1 > cgn_pval ~ TRUE,
          TRUE ~ FALSE
        )
      ) %>%
      dplyr::select( Probe_ID, MG,MR,UG,UR,col, mask )
    # ses_cgn_tib %>% dplyr::filter( mask ) %>% base::nrow()
    
    cpp_man_tib1 <- NULL
    cpp_man_tib1 <- dplyr::bind_rows( ses_cgn_tib,ses_ctl_tib1 ) %>%
      dplyr::mutate(
        MG = dplyr::case_when( col=="2" ~ NA_real_, TRUE ~ MG ),
        MR = dplyr::case_when( col=="2" ~ NA_real_, TRUE ~ MR )
      )
    # cpp_man_tib1 %>% dplyr::filter( mask ) %>% base::nrow()
    
    # ses_cgn_list[[outliers_str]] <- NULL
    # ses_cgn_list[[outliers_str]] <- ses_cgn_tib
    
    cpp_sdf <- NULL
    cpp_sdf <- sesame::SigDF( 
      df = cpp_man_tib1 %>% as.data.frame(), 
      platform = "" )
    cpp_tib <- NULL
    cpp_tib <- cpp_sdf %>% as.data.frame() %>% tibble::as_tibble()
    cpp_sdf %>% sesame::controls() %>% head()
    cpp_sdf %>% dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>% head()
    
    cpp_negs_sdf <- NULL
    cpp_negs_sdf <- mutate_sdf_simple( 
      sdf = cpp_sdf,
      steps = "N", # Without masking...
      negs_min = -1.0, # neg_pval, # negs_min = 0.05,
      poob_min = -1.0, # poob_min = 0.05,
      probe_id = "Probe_ID", beta_key = "Pval",
      ret_tib = FALSE,
      vb=vb, vt=vt, tc=tc )
    
    # Mask Count 1703/293649
    # 1.00 =>   5317 [all ctl]:  13410
    # 0.01 =>  12375
    # 0.05 =>   9135
    # -1.0 => 297843 [all ctl]: 301742
    cpp_negs_msk_cnt <- cpp_negs_sdf %>% dplyr::filter( mask ) %>% base::nrow()
    
    cpp_negs_tib <- NULL
    cpp_negs_tib <- mutate_sdf_simple( 
      sdf = cpp_sdf,
      steps = "n", # Without masking...
      negs_min = 0.05, # negs_min = 0.05,
      poob_min = 0.05, # poob_min = 0.05,
      probe_id = "Probe_ID", beta_key = "Pval",
      ret_tib = TRUE,
      vb=vb, vt=vt, tc=tc )
    
    # A tibble: 129,337 × 2
    cpp_negs_zro_cnt <- cpp_negs_tib %>% dplyr::filter( Pval != 0 ) %>% base::nrow()
    cpp_negs_pvl_cnt <- cpp_negs_tib %>% dplyr::filter( Pval > 0.05 ) %>% base::nrow()
    ...
    cpp_sdf <- NULL
    cpp_sdf <- sesame::SigDF( 
      df = ses_cgn_tib %>% as.data.frame(), 
      platform = "",
      ctl = ses_ctl_tib0 %>% as.data.frame() )
    ...
    cpp_sdf <- sesame::SigDF( 
      ...
      platform = "",
      ctl = ses_ctl_tib %>%
        # dplyr::filter( Stat_Idx==0 ) %>%
        dplyr::filter( Stat_Idx==1 ) %>%
        as.data.frame() )
    ...
    ses_sdf <- NULL
    ses_sdf <- sesame::readIDATpair( prefix.path = sentrix_list[sent_vec][[1]]$Sentrix_Path, 
                                     manifest = msa_man_tib %>% dplyr::filter( !Probe_ID %>% stringr::str_starts("ct") ), 
                                     controls = ses_ctl_tib %>% dplyr::filter( Stat_Idx==1 ) %>% as.data.frame() )
    
    ses_tib <- ses_sdf %>% as.data.frame() %>% tibble::as_tibble()
    
    ses_sdf %>% head()
    ses_sdf %>% sesame::controls() %>% head()
    ses_sdf %>% sesame::controls() %>% dplyr::filter( Type == "NEGATIVE" ) %>% tibble::as_tibble()
    
    # Mask Count 0/293649
    ses_sdf %>% dplyr::filter( mask ) %>% base::nrow()
    
    ses_negs_tib <- NULL
    ses_negs_tib <- mutate_sdf_simple( 
      sdf = ses_sdf,
      steps = "n", # Without masking...
      negs_min = 0.01, # negs_min = 0.05,
      poob_min = 0.01, # poob_min = 0.05,
      probe_id = "Probe_ID", beta_key = "Pval",
      ret_tib = TRUE,
      vb=vb, vt=vt, tc=tc )
    
    # A tibble: 1,759 × 2
    ses_negs_tib %>% dplyr::filter( Pval != 0 )
    
    ses_negs_sdf <- NULL
    ses_negs_sdf <- mutate_sdf_simple( 
      sdf = ses_sdf,
      steps = "N", # Without masking...
      negs_min = 0.00, # neg_pval, # negs_min = 0.05,
      poob_min = 0.00, # poob_min = 0.05,
      probe_id = "Probe_ID", beta_key = "Pval",
      ret_tib = FALSE,
      vb=vb, vt=vt, tc=tc )
    
    # Mask Count 1703/293649
    ses_negs_sdf %>% dplyr::filter( mask ) %>% base::nrow()
    ses_negs_sdf %>% sesame::controls() %>% dplyr::filter( Type == "NEGATIVE" ) %>% tibble::as_tibble()

    ses_poob_tib <- NULL
    ses_poob_tib <- mutate_sdf_simple( 
      sdf = ses_sdf,
      steps = "O", # Without masking...
      negs_min = 0.01, # negs_min = 0.05,
      poob_min = 0.01, # poob_min = 0.05,
      probe_id = "Probe_ID", beta_key = "Pval",
      ret_tib = TRUE,
    ...
    # A tibble: 293,641 × 2
    ses_poob_tib %>% dplyr::filter( Pval != 0 )
    
    ses_poob_sdf <- NULL
    ses_poob_sdf <- mutate_sdf_simple( 
      sdf = ses_sdf,
      steps = "P", # Without masking...
      negs_min = 0.01, # neg_pval, # negs_min = 0.05,
      poob_min = 0.01, # poob_min = 0.05,
      probe_id = "Probe_ID", beta_key = "Pval",
      ret_tib = FALSE,
      vb=vb, vt=vt, tc=tc )
    ...
    # Mask Count 0/293649
    ses_poob_sdf %>% dplyr::filter( mask ) %>% base::nrow()
    # ses_poob_sdf %>% sesame::controls() %>% dplyr::filter( Type == "NEGATIVE" ) %>% tibble::as_tibble()
    
    
    # [Done]: Run with both outliers (rm,ok)
    # [Done]: Finish sdf_to_sesame_ctls() ** Need Sesame Format **
    # [TBD]: Move rm_outliers loop into read_idat_pair_r()
    # [TBD]: Finish sdf_to_sesame_body() ** Need Sesame Format **
    # [TBD]: Fix auto sample sheet name to have a "_" instead of "." in read_idat_pair_r()
    # [TBD]: Test: "ELiminate BAckground-dominated Reading (ELBAR)"
...
if ( FALSE ) {
  ...
  neg_plot_tib <- dplyr::bind_rows(
    dplyr::select( tmp_ctl_dat$neg_tib,Probe_ID,UG,UG_Det_0,mask) %>% 
      dplyr::rename( Sig=UG,Pval=UG_Det_0) %>% dplyr::mutate(col="Grn"),
    dplyr::select( tmp_ctl_dat$neg_tib,Probe_ID,UR,UR_Det_0,mask) %>% 
      dplyr::rename( Sig=UR,Pval=UR_Det_0) %>% dplyr::mutate(col="Red"),
    NULL
  ) %>% dplyr::arrange( Probe_ID )
  
  neg_plot_tib %>% 
    dplyr::filter( Pval > 0.90 ) %>%
    dplyr::group_by( mask,col ) %>%
    dplyr::summarise( 
      Tot_Cnt = n(),
      Sigs_Min=min( Sig,na.rm=TRUE),
      Sigs_Avg=mean( Sig,na.rm=TRUE),
      Sigs_Med=median( Sig,na.rm=TRUE),
      Sigs_Max=max( Sig,na.rm=TRUE),
      
      Pval_Min=min( Pval,na.rm=TRUE),
      Pval_Avg=mean( Pval,na.rm=TRUE),
  ...
  neg_plot_tib %>% 
    dplyr::filter( Pval >= 0.00 ) %>%
    ggplot2::ggplot( aes( x=Sig, color=col) ) +
  ...
  neg_plot_tib %>% 
    dplyr::filter( Pval >= 0.95 ) %>%
    ggplot2::ggplot( aes( x=Pval, y=log(Sig), color=col) ) +
    ggplot2::geom_point()
  
  neg_plot_tib %>% 
    dplyr::filter( Pval >= 0.05 ) %>%
    ggplot2::ggplot( aes( x=Pval, y=Sig ) ) +
    ggplot2::geom_density2d() +
    ggplot2::facet_grid( rows = vars(col) )
}
...
if ( par$version == 5 ) {
  ...
  dB_min_all_csv <- file.path( opt$out_path, paste0( 
    opt$run_name,"_",opt$steps,"_pre-pval-",opt$min_pval,"_dB_min_all.csv.gz") )
  dB_max_all_csv <- NULL
  dB_max_all_csv <- file.path( opt$out_path, paste0( 
    opt$run_name,"_",opt$steps,"_min-pval-1.0_dB_max_all.csv.gz") )
  auto_ssh_rds <- NULL
  auto_ssh_rds <- file.path( opt$out_path, paste0( opt$run_name,"_pre-pval-",opt$pre_pval,"_auto_ssh.rds") )
  
} else {
  dB_min_all_csv <- NULL
  dB_min_all_csv <- file.path( opt$out_path, "dB_min_all.csv.gz" )
  dB_max_all_csv <- NULL
  dB_max_all_csv <- file.path( opt$out_path, "dB_max_all.csv.gz" )
  auto_ssh_rds <- NULL
  auto_ssh_rds <- file.path( opt$out_path, paste0( opt$run_name, "_auto_ssh.rds") )
}

sub_dB_min_all_csv <- NULL
sub_dB_min_all_csv <- file.path( opt$out_path, "dB_min_all.subcols.csv.gz" )
sub_dB_max_all_csv <- NULL
sub_dB_max_all_csv <- file.path( opt$out_path, "dB_max_all.subcols.csv.gz" )
...
#
# [TBD]: Investigate idat_pairs_functions.cpp:
#        - Normal EPIC Run
#        - MSA Run
#        - Workflow and Negative controls
#        - Investigate the Bi,DN,Ne control probes
#
# [TBD]: Investigate current results
#
# [TBD]: Parse [CO][TB][12][Rep] and rebuild sub_data_all.rds
#
# [TBD]: Genotyping:
#        - Platinum Genomes Data
#        - Array Genotyping Data
#        - Complete imGenotyper
#

sub_dB_low_all_csv <- NULL
sub_dB_low_all_csv <- "/Users/bbarnes/Documents/scratch/stable_imSesameCpp_MSAv1_SNV/MSAv10-NCBI-v2/dB_min_all.subcols.csv.gz"

opt$sub_size_dat <- FALSE
opt$sub_size_dat <- TRUE
if ( opt$sub_size_dat && par$version == 5 ) {
  
  cur_data_tib <- NULL
  cur_data_tib <- dplyr::bind_rows(
    readr::read_csv( file = dB_max_all_csv, show_col_types = FALSE ) %>%
      clean_tib(),
    readr::read_csv( file = dB_min_all_csv, show_col_types = FALSE ) %>%
      clean_tib()
  )
  
  # sub_dir <- safe_mkdir( file.path( opt$top_path, "Projects.new/MSA/subs", opt$version ) )
  # cur_data_tib <- NULL
  # cur_data_tib <- dplyr::bind_rows(
  #   readr::read_csv( file = file.path( sub_dir, "dB_min_all.subcols.pval-1.0.csv" ), show_col_types = FALSE ) %>%
  #     clean_tib(),
  #   readr::read_csv( file = file.path( "/Users/bbarnes/mylocal/Documents/Projects.new/MSA/subs/dB_min_all.subcols.pval-0.05.csv" ), show_col_types = FALSE ) %>%
  #     clean_tib(),
  #   readr::read_csv( file = file.path( "/Users/bbarnes/mylocal/Documents/Projects.new/MSA/subs/dB_min_all.subcols.pval-0.1.csv" ), show_col_types = FALSE ) %>%
  #     clean_tib()
  # )
  
  
} else if ( opt$sub_size_dat ) {
  cur_data_tib <- NULL
  if ( file.exists(sub_dB_min_all_csv) &&
       file.exists(sub_dB_low_all_csv) &&
       file.exists(sub_dB_max_all_csv) ) {
    
    cur_data_tib <- NULL
    cur_data_tib <- dplyr::bind_rows(
      readr::read_csv( file = sub_dB_max_all_csv, show_col_types = FALSE ) %>%
        clean_tib(),
      readr::read_csv( file = sub_dB_min_all_csv, show_col_types = FALSE ) %>%
        clean_tib(),
      readr::read_csv( file = sub_dB_low_all_csv, show_col_types = FALSE ) %>%
        clean_tib()
    )
    ...
    cur_data_tib <- NULL
    cur_data_tib <- dplyr::bind_rows(
      readr::read_csv( file = file.path( "/Users/bbarnes/mylocal/Documents/Projects.new/MSA/subs/dB_min_all.subcols.pval-1.0.csv" ), show_col_types = FALSE ) %>%
        clean_tib(),
      readr::read_csv( file = file.path( "/Users/bbarnes/mylocal/Documents/Projects.new/MSA/subs/dB_min_all.subcols.pval-0.05.csv" ), show_col_types = FALSE ) %>%
        clean_tib(),
      readr::read_csv( file = file.path( "/Users/bbarnes/mylocal/Documents/Projects.new/MSA/subs/dB_min_all.subcols.pval-0.1.csv" ), show_col_types = FALSE ) %>%
    ...
    # Load data and write subsets::
    pval_vec <- c("pval-1.0","pval-0.05")
    pval_vec <- c("pval-1.0","pval-0.1")
    ...
    if ( !file.exists(sub_dB_min_all_csv) ||
      ...
      for ( pval_str in pval_vec ) {
        min_pval = NULL
        cur_data_tib <- NULL
        if ( pval_str == "pval-1.0" )  {
          cur_data_tib <- readr::read_csv( file = dB_max_all_csv, show_col_types = FALSE )
          min_pval = 1.0
        } else if ( pval_str == "pval-0.1" )  {
          cur_data_tib <- readr::read_csv( file = dB_min_all_csv, show_col_types = FALSE )
          min_pval = 0.1
        } else if ( pval_str == "pval-0.05" ) {
          cur_data_tib <- readr::read_csv( file = dB_min_all_csv, show_col_types = FALSE )
          min_pval = 0.05
        }
        
        if ( is.null(min_pval) ) {
          stop( glue::glue("[{perrs}]: Min Pval is null!{RET2}") )
          break
        }
        if ( p1 ) cat(glue::glue("{pmssg} Loding dV Min All Data CSV = '{dB_min_all_csv}'{RET}"))
        
        # Update Fields::
        cur_data_tib <- cur_data_tib %>% 
          dplyr::mutate( 
            min_pval = min_pval,
        ...
        if ( pval_str == "pval-1.0" ) {
          readr::write_csv( x = cur_data_tib, file = sub_dB_max_all_csv )
          if ( p1 ) cat(glue::glue("{pmssg} Writing Sub dV Max All Data CSV = '{sub_dB_max_all_csv}'{RET}"))
        }
        if ( pval_str == "pval-0.1" ) {
          readr::write_csv( x = cur_data_tib, file = sub_dB_min_all_csv )
          if ( p1 ) cat(glue::glue("{pmssg} Writing Sub dV Min All Data CSV = '{sub_dB_min_all_csv}'{RET}"))
        }
...
# Screen the Data::
#
if ( !is.null(cur_data_tib) ) {
  
  # Quick summary
  cur_data_sum <- NULL
  cur_data_sum <- cur_data_tib %>% 
    print_sum( vec = c("min_pval","Remove_Outliers","Grp","Sam","Con"),
               vb=vb,vt=vt,tc=tc )
  
  # Probe Type Break Down::
  # cur_data_tib %>% dplyr::filter( Probe_Type != "cg" ) %>% dplyr::filter( Probe_Type != "rs" ) %>% dplyr::filter( Probe_Type != "ch" ) %>% dplyr::filter( Probe_Type != "ct" ) %>% dplyr::pull( Probe_ID ) %>% length()
  
  #
  # Screening:: 250ng
  #
  sub_data_tib <- NULL
  sub_data_tib <- cur_data_tib %>% 
    dplyr::filter( Con == 250 ) %>%
    dplyr::select( Probe_ID,Probe_Type,
                   min_pval,Remove_Outliers,
                   Experiment,Grp,Sam,Con,
                   # dB_tot_cnt,
                   db_pas_per,dB_med,wS_per )
  ...
  if ( FALSE ) {
    ...
    # Density Plots:: Tittration
    T_med_wS_den_ggg <- NULL
    T_med_wS_den_ggg <- cur_data_tib %>% 
      dplyr::filter( Con == 250 ) %>%
      dplyr::filter( Grp == "T" ) %>%
      dplyr::group_by( Probe_Type, min_pval,Remove_Outliers,
                       Grp,
                       Sam,
                       Con ) %>%
      dplyr::filter( Probe_Type == "cg" | Probe_Type == "ch" | Probe_Type == "rs" ) %>%
      ggplot2::ggplot( aes( x=dB_med, color=Probe_Type ) ) + 
      ggplot2::geom_density( alpha = 0.2 ) + 
      ggplot2::facet_grid( rows = vars(min_pval,Remove_Outliers,Con),
                           cols = vars(Sam) ) +
      labs( title = paste0("Titration Experiment. Concentration = 250") )
    T_med_wS_den_ggg
    
    # Density Plots:: Replicates
    L_med_wS_den_ggg <- NULL
    L_med_wS_den_ggg <- cur_data_tib %>% 
      dplyr::filter( Con == 250 ) %>%
      dplyr::filter( Grp == "L" ) %>%
      dplyr::group_by( Probe_Type, min_pval,Remove_Outliers,
                       Grp,
      ...
      ggplot2::facet_grid( rows = vars(min_pval,Remove_Outliers,Con),
                           cols = vars(Sam) ) +
      labs( title = paste0("Replication Experiment. Concentration = 250") )
    L_med_wS_den_ggg
  ...
  if ( FALSE ) {
    ...
    rep_data_sel <- sub_data_tib %>% 
      ...
      dplyr::summarise( Tot = n(),
                        Pas_Avg = mean( db_pas_per, na.rm = TRUE ),
                        Med_Avg = mean( dB_med, na.rm = TRUE ),
                        wSP_Avg = mean( wS_per, na.rm = TRUE ),
                        
                        # Pas_Med = median( db_pas_per, na.rm = TRUE ),
                        # Med_Med = median( dB_med, na.rm = TRUE ),
                        # wSP_Med = median( wS_per, na.rm = TRUE ),
                        
                        Pas_Tot = sum( db_pas_per > 90, na.rm = TRUE ),
                        Pas_Per = round( 100*Pas_Tot/Tot, 1),
                        
                        Med_Tot = sum( dB_med > 0.1, na.rm = TRUE ),
                        Med_Per = round( 100*Med_Tot/Tot, 1),
                        
                        wSP_Tot = sum( wS_per > 90, na.rm = TRUE ),
                        wSP_Per = round( 100*wSP_Tot/Tot, 1),
                        
                        .groups = "drop" ) %>%
      dplyr::arrange( Probe_Type,Grp,-Pas_Avg )
    
    rep_data_sel %>% print(n=1000)
    
    #
    # Titration::
    #
    uhm_data_sel <- NULL
    uhm_data_sel <- sub_data_tib %>% 
      dplyr::filter( Grp == "T" ) %>%
      dplyr::group_by( Probe_Type, min_pval,Remove_Outliers,
                       Grp,
                       Sam,
    ...
    uhm_data_sel %>% print(n=1000)  
  }
  
  # Number Passing X for {db_pas_per,dB_med,wS_per}
  
  uhm_pval <- 1.0
  rep_pval <- 0.1
  
  wS_min_mat <- NULL
  wS_min_mat <- base::matrix( nrow = 2, ncol = 3 )
  wS_min_mat[1,1] <- 25
  wS_min_mat[1,2] <- 10
  wS_min_mat[1,3] <-  5
  wS_min_mat[2,1] <- 75
    ...
    for ( metric_str in metric_vec ) {
      for ( rm_out in c(TRUE,FALSE) ) {
        for ( ptype in ptype_vec ) {
          ...
          # A tibble: 283,133 × 1
          uhm_data_tib <- NULL
          uhm_data_tib <- cur_data_tib %>% # sub_data_tib %>%
            dplyr::filter( Probe_Type == ptype ) %>%
            dplyr::filter( min_pval == uhm_pval ) %>%
          ...
          if ( metric_str == "wS_per" ) {
            ...
            t1_tib <- dplyr::filter( uhm_data_tib, Grp == "T", Sam == "UM" & db_pas_per > pp_um_min )
            t2_tib <- dplyr::filter( uhm_data_tib, Grp == "T", Sam == "UH" & db_pas_per > pp_uh_min )
            t3_tib <- dplyr::filter( uhm_data_tib, Grp == "T", Sam == "HM" & db_pas_per > pp_hm_min )
          } else {
          ...
          # A tibble: 283,133 × 1
          rep_data_tib <- NULL
          rep_data_tib <- cur_data_tib %>% # sub_data_tib %>%
            dplyr::filter( Probe_Type == ptype ) %>%
            dplyr::filter( min_pval == rep_pval ) %>%
            dplyr::filter( Remove_Outliers == rm_out ) %>%
            dplyr::filter( Con == 250 )
          ...
          lH_tib <- dplyr::filter( rep_data_tib, Grp == "L", Sam == "H" & wS_per > pp_min )
          lJ_tib <- dplyr::filter( rep_data_tib, Grp == "L", Sam == "J" & wS_per > pp_min )
          lK_tib <- dplyr::filter( rep_data_tib, Grp == "L", Sam == "K" & wS_per > pp_min )
          lM_tib <- dplyr::filter( rep_data_tib, Grp == "L", Sam == "M" & wS_per > pp_min )
          lR_tib <- dplyr::filter( rep_data_tib, Grp == "L", Sam == "R" & wS_per > pp_min )
          ...
          fin_sel_cnt <- NULL
          fin_sel_cnt <- uhm_sel_data_tib %>% 
            dplyr::filter( Probe_ID %in% rep_sel_data_tib$Probe_ID ) %>%
            base::nrow()
          
          fin_sel_per <- NULL
          fin_sel_per <- fin_sel_cnt / uhm_data_cnt
          
          if ( p1 ) cat(glue::glue(
            "{pmssg} [{ptype}] ",
            "metric={metric_str} ",
            "rm-outliers={rm_out}, ",
            "ws_mins=",wS_min_mat[mm, ] %>% paste( collapse = "," ),", ",
            "pp_mins=",pp_min,", ",
            "{fin_sel_cnt}/{uhm_data_cnt} = {fin_sel_per}.{RET}"))
        }
      }
    }
...
if ( FALSE ) {
  ...
  # [cg] 283133 - 274267 = 8866
  
  # A tibble: 283,133 × 1
  sel_data_tib <- NULL
  sel_data_tib <- sub_data_tib %>%
    dplyr::filter( Probe_Type == "ch" ) %>%
    dplyr::filter( min_pval == 1.0 ) %>%
  ...
  UH_min <- 0
  HM_min <- 0
  t1_tib <- dplyr::filter( sel_data_tib, Sam == "UM" & wS_per > UM_min ) # 75
  t2_tib <- dplyr::filter( sel_data_tib, Sam == "UH" & wS_per > UH_min ) # 30
  t3_tib <- dplyr::filter( sel_data_tib, Sam == "HM" & wS_per > HM_min ) # 15
  
  # A tibble: 64 × 1
  sel_data_tib %>% dplyr::distinct( Probe_ID ) %>%
    dplyr::filter( Probe_ID %in% t1_tib$Probe_ID ) %>%
    dplyr::filter( Probe_ID %in% t2_tib$Probe_ID ) %>%
    dplyr::filter( Probe_ID %in% t3_tib$Probe_ID )
  ...
  # Plotting
  #
  run_quick_plots <- TRUE
  run_quick_plots <- FALSE
  
  if ( run_quick_plots ) {
    sub_plot_tib <- NULL
    sub_quick_plots <- TRUE
    if ( sub_quick_plots ) {
      
      sub_exp_list <- NULL
      sub_exp_list <- sub_data_tib %>% 
        # dplyr::filter( Grp == "T" ) %>%
        split(.$Experiment)
      
      sub_plot_tib <- NULL
      for ( exp in names(sub_exp_list) ) {
        sub_plot_tib <- sub_plot_tib %>% dplyr::bind_rows( 
          sub_exp_list[[exp]] %>% dplyr::filter( min_pval == 1.0  ) %>% head(n=20000),
          sub_exp_list[[exp]] %>% dplyr::filter( min_pval == 0.05 ) %>% tail(n=20000)
        )
      }
      
    } else {
      sub_plot_tib <- sub_data_tib
    }
    
    #
    # 2D Density:: Replicate ( dB_med x wS_per)
    #
    L_med_wS_d2d_ggg <- NULL
    L_med_wS_d2d_ggg <- sub_plot_tib %>% 
    ...
    #
    # Scatter Plot:: Replicate ( dB_med x wS_per)
    #
    L_med_wS_pnt_ggg <- NULL
    L_med_wS_pnt_ggg <- sub_plot_tib %>% 
      # dplyr::filter( Probe_Type == "cg" ) %>%
      dplyr::filter( Grp == "L" ) %>%
      ggplot2::ggplot( aes( x=dB_med, y=wS_per, color=Probe_Type ) ) +
      ggplot2::geom_point() + 
      ggplot2::facet_grid( rows = vars(min_pval,Remove_Outliers,Con), 
                           cols = vars(Sam) )
    L_med_wS_pnt_ggg
    ...
    #
    # 2D Density:: Replicate ( db_pas_per x wS_per)
    #
    L_pas_wS_d2d_ggg <- NULL
    L_pas_wS_d2d_ggg <- sub_plot_tib %>% 
      dplyr::filter( Probe_Type == "cg" ) %>%
      dplyr::filter( Grp == "L" ) %>%
    ...
    # 2D Density:: Titration ( dB_med x wS_per)
    T_med_wS_d2d_ggg <- NULL
    T_med_wS_d2d_ggg <- sub_plot_tib %>% 
      dplyr::filter( Probe_Type == "cg" ) %>%
      dplyr::filter( Grp == "T" ) %>%
      ggplot2::ggplot( aes( x=dB_med, y=wS_per ) ) +
      ggplot2::geom_density2d() +
    ...
    # Scatter Plot:: Titration ( dB_med x wS_per)
    T_med_wS_pnt_ggg <- NULL
    T_med_wS_pnt_ggg <- sub_plot_tib %>% 
      dplyr::filter( Probe_Type == "cg" ) %>%
      dplyr::filter( Grp == "T" ) %>%
      ggplot2::ggplot( aes( x=dB_med, y=wS_per, color=Probe_Type ) ) +
      ggplot2::geom_point() + 
...
if ( FALSE ) {
  ...
  for ( rm_outliers in rm_outliers_vec ) {
    ...
    uhm_med_wS_d2d_ggg <- NULL
    uhm_med_wS_d2d_ggg <- cur_uhm_data_tib %>% 
      ggplot2::ggplot( aes( x=dB_med, y=wS_per ) ) +
      ggplot2::geom_density2d() +
      ggplot2::geom_point() + 
      ggplot2::facet_grid( rows = var(Remove_Outliers), 
                           cols = vars(Experiment) )
    uhm_med_wS_d2d_ggg
    
    exp_list <- cur_uhm_data_tib %>% split(.$Experiment)
    sub_uhm_data_tib <- NULL
    for ( exp in names(exp_list) ) {
      
      sub_uhm_data_tib <- sub_uhm_data_tib %>%
        dplyr::bind_rows( exp_list[[exp]] %>% head(n=10000) )
    }
    
    uhm_med_wS_d2d_ggg <- NULL
    uhm_med_wS_d2d_ggg <- sub_uhm_data_tib %>% 
      ggplot2::ggplot( aes( x=dB_med, y=wS_per, color=Probe_Type ) ) +
      ggplot2::geom_density2d() +
      ggplot2::geom_point() + 
...
msa_ssh_fmt_sum <- NULL
msa_ssh_fmt_sum <- msa_ssh_tib %>%
  print_sum( vec = c("Source_Key","User_Format"),
             vb=vb,vt=vt+3,tc=tc+1, tt=tt )

msa_ssh_chp_sum <- NULL
msa_ssh_chp_sum <- msa_ssh_tib %>% 
  dplyr::filter( Source_Key == "R0" ) %>% 
  tidyr::separate( Sentrix_Name, into=c("Sentrix_ID","Sentrix_POS"), sep="_" ) %>% 
  print_sum( vec = c("Sentrix_ID"), vb=vb,vt=vt+3,tc=tc )

msa_ssh_row_sum <- NULL
msa_ssh_row_sum <- msa_ssh_tib %>% 
  dplyr::filter( Source_Key == "R0" ) %>% 
  tidyr::separate( Sentrix_Name, into=c("Sentrix_ID","Sentrix_POS"), sep="_" ) %>% 
  print_sum( vec = c("Sentrix_POS"), vb=vb,vt=vt+3,tc=tc )

# [TBD]: Update Source_Key
# [TBD]: Add Platform (or can we just preeict it), look at Rccp output...
# [TBD]: Rename EX24 => 24x1
...
epi_ssh_fmt_sum <- NULL
epi_ssh_fmt_sum <- epi_ssh_tib %>%
  print_sum( vec = c("Source_Key","User_Format"),
             vb=vb,vt=vt+1,tc=tc+1, tt=tt )
...
ssh_dir <- safe_mkdir( dir = file.path( opt$out_path, "sample_sheets" ) )
ssh_csv <- file.path( ssh_dir, "MAS.v.1.0_sample_sheet.csv.gz" )
readr::write_csv( x = ssh_tib, file = ssh_csv )
...
sel_ssh_tib <- dplyr::bind_rows(
  ...
  ssh_tib %>% dplyr::filter( Source_Key=="T1" & Sample_Group_Key=="T" & Sample_Base_Key=="E" ) %>%
    ...
    dplyr::mutate( Exp_Str = paste( Source_Key,"UH",Sample_Input, sep="_" ) ) %>%
    dplyr::select( Exp_Str, dplyr::everything() ),
  ...
  ssh_tib %>% dplyr::filter( Source_Key=="T1" & Sample_Group_Key=="T" & Sample_Base_Key=="E" ) %>%
    ...
    dplyr::filter( Sample_Titration ==  0 | Sample_Titration ==  100 ) %>% 
    dplyr::filter( Sample_Input == 50 ) %>%
    dplyr::mutate( Exp_Str = paste( Source_Key,"UM",Sample_Input, sep="_" ) ) %>%
    dplyr::select( Exp_Str, dplyr::everything() ),
  ...
  ssh_tib %>% dplyr::filter( Source_Key=="T1" & Sample_Group_Key=="T" & Sample_Base_Key=="E" ) %>%
    ...
    dplyr::filter( Sample_Titration ==  50 | Sample_Titration ==  100 ) %>% 
    dplyr::filter( Sample_Input == 50 ) %>%
    dplyr::mutate( Exp_Str = paste( Source_Key,"HM",Sample_Input, sep="_" ) ) %>%
    dplyr::select( Exp_Str, dplyr::everything() ),
...
sel_ssh_list <- sel_ssh_tib %>% 
  dplyr::filter( Source_Key == "T1" ) %>% split( .$Exp_Str )
...
# ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
#
#                       Reload Pre-SDFs from scratch::
#
# ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #

#
# NOTE: Not really sure we need to load everything at once...
#
if ( FALSE ) {
  ...
  for ( rm_outliers in rm_outliers_vec ) {
    ...
    pre_sdf_dats[[outliers_str]] <- NULL
    pre_sdf_dats[[outliers_str]] <- file_list( 
      path    = file.path( pre_sdf_path, "read_idat_pair_r" ), 
      prefix  = file.path( pre_sdf_path, "read_idat_pair_r" ),
      suffix  = paste0( "_",outliers_str,".read_idat_pair_r.rds" ),
      pattern = paste0( "_",outliers_str,".read_idat_pair_r.rds$" ),
      
      vb=vb,vt=vt+1,tc=tc ) %>% # head() %>%
...
if ( !opt$run_sesame && 
     file.exists( dB_min_all_csv ) &&
     file.exists( dB_max_all_csv ) &&
  ...
  dB_min_all_tib <- readr::read_csv( file = dB_min_all_csv )
  if ( p1 ) cat(glue::glue("{pmssg} Loding dV Min All Data CSV = '{dB_min_all_csv}'{RET}"))
  
  dB_max_all_tib <- readr::read_csv( file = dB_max_all_csv )
  ...
  for ( rm_outliers in rm_outliers_vec ) {
    ...
    sdf_paths <- NULL
    sdf_paths <- list.files( path = file.path( opt$out_path, "pre_sdf",outliers_str ), pattern = paste0("_",outliers_str,".read_idat_pair_r.rds$"), recursive = TRUE, full.names = TRUE )
    probe_map <- NULL
    ...
    for ( exp_name in names(sel_ssh_list) ) {
      ...
      for ( sentrix_name in names(ssh_sel_list) ) {
        ...
        beta_tib <- mutate_sdf_simple( 
          ...
          # For V,v,W,w negs_min and poob_min don't matter...
        ...
        poob_tib <- mutate_sdf_simple( 
          sdf = work_sdf,
          steps = "O", 
          # For O or o negs_min and poob_min don't matter because you're returning the full values...
          negs_min = 1.0,
          poob_min = 1.0,
          vb=vb, vt=vt, tc=tc ) %>% 
          as.data.frame() %>% 
        ...
        cur_auto_ssh_tib <- cur_pre_ssh_tib %>% dplyr::filter( Sentrix_Name == sentrix_name ) %>%
          ...
          dplyr::mutate( Remove_Outliers = rm_outliers, 
                         Experiment = exp_name,
                         Pass_Poob_Cnt = which(  poob_mat <= opt$min_pval ) %>% length(),
                         Fail_Poob_Cnt = which( !poob_mat <= opt$min_pval ) %>% length(),
      ...
      cur_dB_max_all_tib <- NULL
      cur_dB_max_all_tib <- calc_dBs( beta_tibA = all_beta_tibs[[1]],
                                      pval_tibA = all_poob_tibs[[1]],
                                      name_strA = exp_name1, # names(all_beta_tibs[[1]])[1]
      ...
      #
      # OLD Plotting Scratch to be deleted....
      #
      if ( FALSE ) { 
        
        cur_dB_min_all_tib <- NULL
        cur_dB_min_all_tib <- cur_dB_min_all_tib %>% 
          dplyr::rename( Probe_Idx = Probe_ID ) %>%
          dplyr::mutate( Probe_Idx = Probe_Idx %>% as.integer() ) %>%
          dplyr::inner_join( probe_map, by=c("Probe_Idx") ) %>%
          dplyr::mutate( Probe_Type = Probe_ID %>% stringr::str_sub(1,2) ) %>%
        ...
        cur_dB_cgn_tib <- NULL
        cur_dB_cgn_tib <- cur_dB_prb_tib %>%
          dplyr::filter( Probe_Type == "cg" )
        # dplyr::filter( Probe_ID %>% stringr::str_starts("cg") )
        
        # cur_dB_cgn_val = 0.4675781
        cur_dB_cgn_val <- NULL
        cur_dB_cgn_val <- stats::cor( 
          x = cur_dB_cgn_tib$wS_per, 
          y = cur_dB_cgn_tib$db_pas_per, 
          use = "pairwise.complete.obs", method = "pearson" )
        print(cur_dB_cgn_val)
        
        # cur_dB_cgn_val = 
        cor_pas_val <- stats::cor( 
          x = cur_dB_tib$wS_per, 
          # y = cur_dB_tib %>% dplyr::mutate( db_pas_per = dplyr::case_when( !is.finite(db_pas_per) ~ NA, TRUE ~ db_pas_per ) ) %>% dplyr::pull( db_pas_per ) %>% as.vector(), 
          y = cur_dB_tib$db_pas_per, 
          use = "pairwise.complete.obs", method = "pearson" )
        print(cor_pas_val)
        
        cor_pas_val <- stats::cor( 
          # x = cur_dB_tibO %>% dplyr::mutate( db_pas_per = dplyr::case_when( !is.finite(db_pas_per) ~ NA, TRUE ~ db_pas_per ) ) %>% dplyr::pull( db_pas_per ) %>% as.vector(), 
          x = cur_dB_tibO$wS_per, 
          y = cur_dB_tibO$db_pas_per, 
          use = "pairwise.complete.obs", method = "pearson" )
        print(cor_pas_val)
        
        cor_med_val <- cor( x = cur_dB_tib$wS_per, 
                            y = cur_dB_tib$dB_med, 
                            use = "pairwise.complete.obs", method = "pearson" )
        print(cor_med_val)
        
        # BOX Plot: ALL
        box_pas_wS_all_ggg <- NULL
        box_pas_wS_all_ggg <- cur_dB_min_all_tib %>% 
          ggplot2::ggplot( aes(x=db_pas_per,y=wS_per, group = db_pas_per) ) + 
          ggplot2::geom_boxplot( varwidth = TRUE )
        box_pas_wS_all_ggg
        
        # BOX Plot: CGN
        box_pas_wS_cgn_ggg <- NULL
        box_pas_wS_cgn_ggg <- cur_dB_cgn_tib %>% 
          ggplot2::ggplot( aes(x=db_pas_per,y=wS_per, group = db_pas_per) ) + 
          ggplot2::geom_boxplot( varwidth = TRUE )
        box_pas_wS_cgn_ggg
        
        # D2D Plot: ALL
        d2d_med_wS_all_ggg <- NULL
        d2d_med_wS_all_ggg <- cur_dB_tib %>% 
          ggplot2::ggplot( aes( x=dB_med, y=wS_per ) ) +
          ggplot2::geom_density2d()
        d2d_med_wS_all_ggg
        
        # D2D Plot: CGN
        d2d_med_wS_cgn_ggg <- NULL
        d2d_med_wS_cgn_ggg <- cur_dB_cgn_tib %>% 
          ggplot2::ggplot( aes( x=dB_med, y=wS_per ) ) +
          ggplot2::geom_density2d()
        d2d_med_wS_cgn_ggg
        ...
        cbind(
          cbind( c(1,NA_real_,3,NA_real_), c( 3,2,1,NA_real_) ) %>% matrixStats::rowDiffs(),
          cbind( c(3,2,3,NA_real_), c( 1,1,NA_real_,NA_real_) ) %>% matrixStats::rowDiffs()
        ) %>% matrixStats::rowMedians( na.rm = TRUE )
        
        cbind(
          cbind( c(1,NA,3,NA), c( 3,2,1,NA) ) %>% matrixStats::rowDiffs(),
          cbind( c(3,2,3,NA), c( 1,1,NA,NA) ) %>% matrixStats::rowDiffs()
        ) %>% matrixStats::rowMedians( na.rm = TRUE )
        ...
        box_pas_wSum_ggg <- NULL
        box_pas_wSum_ggg <- cur_dB_tib %>% 
          ggplot2::ggplot( aes(x=db_pas_per,y=wS_sum, group = db_pas_per) ) + 
          ggplot2::geom_boxplot( varwidth = TRUE )
        box_pas_wSum_ggg
        ...
        d2d_wSum_med_ggg <- NULL
        d2d_wSum_med_ggg <- cur_dB_tib %>% 
          ggplot2::ggplot( aes( x=wS_sum, y=dB_med ) ) +
          ggplot2::geom_density2d()
        d2d_wSum_med_ggg
        ...
        d2d_wSum_med_ggg2 <- NULL
        d2d_wSum_med_ggg2 <- cur_dB_tib %>% 
          ggplot2::ggplot( aes( x=wS_sum, y=dB_med ) ) +
          ggplot2::geom_density2d() +
          ggplot2::facet_grid(
            rows = vars(db_pas_per)
      ...
      dB_min_all_tib <- dB_min_all_tib %>% 
        dplyr::bind_rows(
          dplyr::mutate( cur_dB_min_all_tib,
      ...
      dB_max_all_tib <- dB_max_all_tib %>% 
        dplyr::bind_rows(
          dplyr::mutate( cur_dB_max_all_tib,
                         Remove_Outliers = rm_outliers, 
                         Experiment = exp_name ) %>% 
            tidyr::separate( Experiment, into=c("Set","Sam","Con"), sep="_", 
                             remove = FALSE, convert = TRUE ) %>%
