...
if ( FALSE ) {
  # msa_all_ctls_tib %>% dplyr::mutate( Probe_ID = Probe_ID %>% stringr::str_remove("-[0-9]+$") ) %>% dplyr::distinct( Probe_ID, .keep_all = TRUE )
  
  msa_dup_ctls_tib <- NULL
  msa_dup_ctls_tib <- msa_all_ctls_tib %>% 
    dplyr::distinct( U, .keep_all = TRUE ) %>%
    dplyr::mutate( Probe_ID2 = Probe_ID %>% stringr::str_remove("-[0-9]+$") ) %>% 
    dplyr::add_count( Probe_ID2, name = "ID_Cnt" ) %>% 
    dplyr::filter( ID_Cnt != 1 ) %>%
    dplyr::arrange( Probe_ID ) %>%
    dplyr::select( Probe_ID, Probe_ID2, ID_Cnt, dplyr::everything() )
  
  msa_dup_ctls_sum <- NULL
  msa_dup_ctls_sum <- msa_dup_ctls_tib %>% print_sum( vec=c("Probe_Type") )
  msa_dup_ctls_sum %>% print(n=1000)
  
  msa_ctl_man_tib <- NULL
  msa_ctl_man_tib <- readr::read_csv( file.path( opt$top_path, "data/manifests/methylation/EX/MSAEX03/MSA-Interm-48v0-3_SS_BP123_A1.csv.gz" ), show_col_types = FALSE ) %>% 
    clean_tib() %>%
    ...
    dplyr::mutate( Probe_ID = Probe_ID %>% stringr::str_replace("^ZZ", "ctl_"),
                   ...
                   Probe_Type = dplyr::case_when(
                     Probe_Type == "HYBRIDIZATION - HIGH" ~ "HYBRIDIZATION-HIGH",
                     
                     Probe_Type == "EXTENSION (A)" ~ "EXTENSION_A",
                     Probe_Type == "EXTENSION (C)" ~ "EXTENSION_C",
                     Probe_Type == "EXTENSION (G)" ~ "EXTENSION_G",
                     Probe_Type == "EXTENSION (T)" ~ "EXTENSION_T",
                     
                     Probe_Type == "NP (A)" ~ "NP_A",
                     Probe_Type == "NP (C)" ~ "NP_C",
                     Probe_Type == "NP (G)" ~ "NP_G",
                     Probe_Type == "NP (T)" ~ "NP_T",
