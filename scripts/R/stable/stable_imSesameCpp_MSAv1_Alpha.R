...
ssh_tib <- readr::read_csv( file = ssh_csv, skip = 7, show_col_types = FALSE ) %>% 
  ...
  dplyr::mutate(
    ...
    Sample_Base = paste( Sample_GroupKey,Sample_NameKey, sep="_" )
  ) %>%
...
opt$write_sample_sheet <- FALSE
opt$write_sample_sheet <- TRUE

opt$ssh_path <- safe_mkdir( file.path( opt$out_path, "sample_sheets" ) )
ssh_clean_csv <- file.path( opt$ssh_path, "Samplesheet_48x1_preDVT_072023-SMG-Verbose.formatted.csv.gz" )

if ( opt$write_sample_sheet ) readr::write_csv( x = ssh_tib, file = ssh_clean_csv )
...
msa_neg_ctls_csv <- file.path( opt$top_path, "data/manifests/methylation/EX/MSAEX03/MSA-Interm-48v0-3_SS_BP123_A1.negs-4255.csv.gz" )
msa_neg_ctls_tib <- NULL
msa_neg_ctls_tib <- msa_ctls_to_negs( msa_all_ctls_tib ) %>%
  # dplyr::bind_rows( neg_ctls_tib ) %>%
  dplyr::arrange( Address ) %>%
  dplyr::distinct( Address, .keep_all = TRUE )
readr::write_csv( x = msa_neg_ctls_tib, file = msa_neg_ctls_csv )
...
#
# [TBD]: Remove or Clean Up one_at_a_time option
# [TBD]: Write Function Wrapper for all this...
# [TBD]: Investigate mcapply error or replace with foreach!
#
opt$build_pre_sdf <- TRUE
opt$build_pre_sdf <- FALSE

if ( opt$build_pre_sdf ) {
...
uhm_train_ssh_tib <- dplyr::bind_rows(
  ...
  uhm_train_ssh_tib0 %>% 
    ...
    dplyr::mutate( Sample_Key  = paste0( Sample_GroupKey,"_HM_",Sample_InputKey), 
                   Sample_Unq2 = paste( Sample_Key,Sample_Rep, sep="_") ) %>%
    dplyr::select( -Sample_Unq2 )
) %>% dplyr::arrange( Sample_Titration ) 
...
# 2. [Oon]
# 3. [P|N][Bb]
opt$work_step_mat <- NULL
opt$work_step_mat <- matrix( 
  c( "DC", "O","PB", # [Done]
    "D", "O","PB",
    "CD", "O","PB",
    "NDC","O","PB",
  ...
  for ( min_pval in opt$min_pval_vec ) {
    ...
    for ( mask_val in opt$mask_vec ) {
      ...
      for ( widx in c(1:base::ncol(opt$work_step_mat)) ) {
        ...
        #
        for ( exp_key in names(train_ssh_tibs) ) {
          if ( p1 ) cat(glue::glue("{pmssg}{TAB2} exp_key = '{exp_key}'{RET}"))
          
          cur_ssh_tibs <- NULL
          cur_ssh_tibs <- train_ssh_tibs[[exp_key]] %>% split(.$Sentrix_Name)
          ...
          for ( sentrix_name in names(cur_ssh_tibs) ) {
            ...
            pre_sdf_tib <- NULL
            pre_sdf_tib <- pre_sdf_list[[outliers_str]][[sentrix_name]]        
            
            sample_base <- NULL
            sample_base <- cur_ssh_tibs[[sentrix_name]]$Sample_Base
            ...
            #
            # [TBD]:: Write Controls!!! OR Controls Summary per Experiment!
            #
            # i.e. ctls_tibs
            #
            debug <- FALSE
            ...
            #
            if ( is.null( beta_tibs[[sample_base]] ) ) { 
              beta_tibs[[sample_base]] <- beta_tib
            } else {
              beta_tibs[[sample_base]] <- beta_tibs[[sample_base]] %>% 
                inner_join( beta_tib, by = c("Probe_ID") )
            }
            
            if ( is.null( pval_tibs[[sample_base]] ) ) {
              pval_tibs[[sample_base]] <- pval_tib
            } else {
              pval_tibs[[sample_base]] <- pval_tibs[[sample_base]] %>% 
                inner_join( pval_tib, by = c("Probe_ID") )
            }
          ...
          #
          # [TBD]: Reduce cur_min_dB to almost zero
          #
          is_uhm <- FALSE
          if ( exp_key == "T_UM_25" ) {
            is_uhm <- TRUE
          ...
          if ( is_uhm ) {
            beta_tibs2 <- beta_tibs[[2]]
            pval_tibs2 <- pval_tibs[[2]]
          }
          ...
          #
          cur_dB_tib <- NULL
          cur_dB_tib <- calc_dBs( beta_tibA = beta_tibs[[1]],
                                  pval_tibA = pval_tibs[[1]],
                                  ...
                                  out_bool = outliers_val,
                                  min_pval = cur_min_pval,
                                  mask_val = mask_val,
                                  work_str = work_full_str,
