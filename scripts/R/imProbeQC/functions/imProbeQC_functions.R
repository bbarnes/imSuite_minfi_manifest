...
calc_dBs = function( beta_tibA,
                     ...
                     out_bool = TRUE,
                     min_pval = 1.0,
                     mask_val = NA_character_,
                     work_str = NA_character_,
                     ...
                     fun_tag='calc_dBs')
{
  ...
  ftime <- base::system.time({
    ...
    if ( cmp_str == "lte" ) {
      ret_tib <- ret_tib %>% 
        dplyr::mutate(
          dB_pas_cnt = matrixStats::rowCounts( dB_mat <= min_dB, na.rm = TRUE ),
    ...
    if ( write_sum ) ret_tib <- ret_tib  %>% 
      dplyr::mutate( Probe_Type = Probe_ID %>% stringr::str_sub(1,2),
                     Remove_Outliers = out_bool,
                     Mask_Index = mask_val,
                     Work_Steps = work_str,
                     Experiment = exp_key ) %>%
      dplyr::select( Probe_ID,Remove_Outliers,Experiment, dplyr::everything() ) %>%
      tidyr::separate( Experiment, into=c("Set","Sam","Con"), sep="_", 
                       remove = FALSE ) %>% #, convert = TRUE,  ) %>%
      dplyr::mutate( 
        Min_Pval = min_pval,
        Con = Con %>% as.integer() * 10 %>% as.integer(),
        db_pas_per = db_pas_per %>% round(2),
        dB_avg = dB_avg %>% round(6),
        dB_med = dB_med %>% round(6),
        mP_avg = mP_avg %>% round(6),
        mP_med = mP_med %>% round(6),
        wS_per = wS_per %>% round(6)
      )  %>% dplyr::select( Probe_ID,Probe_Type,
                            Remove_Outliers,
                            Min_Pval,
                            Mask_Index,
                            Experiment,Set,Sam,Con,
                            dB_tot_cnt,
                            db_pas_per,
                            dB_avg,dB_med,
                            mP_avg,mP_med,
