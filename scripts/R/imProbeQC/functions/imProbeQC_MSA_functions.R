...
#
# [TBD]: Loop over parameters (pval's, workflows_steps, outliers, stats_steps )
# [TBD]: Build single analytical/controls data frame
# [TBD]: --negs_only
#
prefix_to_sdf_rcpp = function( prefix,
...
#
# anno just needs to search for {Infinium I/rs/nv} + col
#  if col == G/R || Probe_ID ~ m/^[rs]/[nv]/ )...
#
formatVCF3 = function(
...
#
# This is just for reference before I start messing with stuff...
#
formatVCF0 = function(
    sdf, anno, vcf=NULL, genome="hg38", verbose = FALSE) {
...
build_core_msa_man <- function( neg_rds = NULL, 
                                m03_csv, m10_csv, out_csv,
                                vb=0,vt=3,tc=1, tt=NULL,
                                fun_tag='build_core_msa_man')
{
  org_neg_tib  <- NULL
  if ( !is.null(neg_rds) ) org_neg_tib  <- readr::read_rds( neg_rds ) %>%
  ...
  ctl_man_tib <- readr::read_csv( file = neg_man_csv, show_col_types = FALSE ) %>% 
    ...
    dplyr::mutate( Probe_ID = Probe_ID %>% stringr::str_replace("^ZZ", "ctl_"),
                   ...
                   Probe_Type = dplyr::case_when(
                     ...
                     Probe_Type == "NEGATIVE" & !Probe_ID %>% stringr::str_starts("ctl_Negative") ~ "OTHER",
