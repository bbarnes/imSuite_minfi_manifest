...
man_tib <- readr::read_csv( file = man_csv, show_col_types = FALSE )
...
prb_all_tab <- NULL
prb_all_tab <- rep_tib %>% dplyr::left_join( tit_tib, by=c("Probe_ID") ) %>% 
  tidyr::pivot_longer( cols = - Probe_ID, 
                       names_to = "Sample", 
                       values_to = "Pass" )
...
# All Summary:
dplyr::left_join( prb_rep_sum, prb_tit_sum, by=c("Probe_ID"), suffix=c("_Rep","_UHM") ) %>%
