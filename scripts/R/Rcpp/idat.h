...
/*  ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- -----
 *
 *  Struct:: Manifest/Sample Match
 *   - Stores Man ( Manifest Name )
 *   - Stores Sam ( Sample Name )
 *   - Stores Tro ( Tango manifest Reciprocal percent Overlap )
 *   - Stores Sr2 ( Sample R-squared )
 *   - Stores SdB ( Sample average Delta Beta )
 * 
 * ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- */

struct idat_cmp_struct {
  _S Man;
  _S Sam;
  _d Tro;
  _d Sr2;
...
// [NOTE]: Below is a failed attempt to create a default Idat vector for constructor overloading...
// static std::vector<Idat> DEFAULT_IDAT_VEC;
  
private:
  
  // [TBD]: Add Auto Match Fields:: add defaults/store as a sorted map of structs?
  // - Orderd array of structs. Two structures?
  //
  // - _S = Man: Manifest
  // - _S = Sam: Sample_Name
  // - _d = Tro: RPO (Reciprocal Percent Overlap) or RMO (Reciprical Manifest Overlap)? Tango: rto, or RO...
  // - _d = Sr2: r2
  // - _d = SdB: dB ? Store as dB vector? Or just adB (average delta Beta)
  //
  std::vector< idat_cmp_struct > cmp_vec;
...
public: 
        ...
        const _S ft="Idat" )
  {
    ...
    // <<_fo<<"\t      Pval_Size = '"<<pval_vec.size()<<"'\n"
    // <<_fo<<"\t      Refs_Size = '"<<refs_vec.size()<<"'\n"
    // << std::endl;
    
    this->parse_idat( idat_path,col,pval_vec, rm_pval_outliers, refs_vec,
                      vb,vt+1,tc );
  ...
  // Contstructor if Reference Idat Vectors are NOT provided::
  Idat( const _S idat_path,
        const _B col,
        const _Y rm_pval_outliers = false,
  ...
  // Contstructor if Reference Idat Vectors are NOT provided::
  // Contstructor if Background Addresses are provided::
  Idat( const _S idat_path,
        const _B col,
        const std::vector<_I>& pval_vec,
        const _Y rm_pval_outliers = false,
        ...
        const _I tc = 0,
        const _S ft = "AS_Idat" ) :
    Idat( idat_path, col,pval_vec,rm_pval_outliers,{}, vb,vt,tc ) {};

  // Contstructor if Reference Idat Vectors are provided::
  // Contstructor if Background Addresses are NOT provided::
  Idat( const _S idat_path,
        const _B col,
        const _Y rm_pval_outliers = false,
        const std::vector<idat::Idat>& ref_vec = {},

        const _I vb = 0,
        const _I vt = 0,
        const _I tc = 0,
        const _S ft = "AS_Idat" ) :
    Idat( idat_path, col,DEFAULT_UINT_VEC,rm_pval_outliers,ref_vec, vb,vt,tc ) {};
                 ...
                 const _S ft="parse_idat" )
  {
    ...
    const _I pval_size = pval_vec.size();
    const _I refs_size = refs_vec.size();
    // const _I Pval_Size = 0;
    // if ( pval_vec.size() != 0 ) Pval_Size = pval_vec.size();
    ...
    /* ***** ***** ***** ***** ***** ***** | ***** ***** ***** ***** ***** ***** 
     * 
     *              Identify Manifest/Sample/Negative Control Set::
     *  
     *  1. Array of reference idats provided?
     *     a. Loop through and calculate: {reciprocal tango overlap(rto), r2,dB}
     *     b. Update internal variables: { manifest, sample, rto,r2,dB, negative ctl set }
     *  2. Negative Control Set Provided?
     * 
     * ***** ***** ***** ***** ***** ***** | ***** ***** ***** ***** ***** ***** */
     ...
     *
     * if refs_vec.size() != 0
     *  1. Scan through refs and then assign best:
     *     - cmp_vec
     *     - pval_vec
     * else
    ...
    // Make Tango/Address Assignments and possible pval Assignments::
    ...
    if ( refs_size != 0 ) {
      ...
      // Loop through refs_vec
      //
      // [TBD]: Make the below prototype into a function...
      //
      _I top_cnt = 0;
      _I top_idx = 0;
      _d top_sr2 = 0.0;
      for ( std::size_t rr=0; rr<refs_size; rr++ ) {
        // - _S = Man: Manifest
        // - _S = Sam: Sample_Name
        // - _d = Tro: RPO (Reciprocal Percent Overlap) or RMO (Reciprical Manifest Overlap)? Tango: rto, or RO...
        // - _d = Sr2: r2
        ...
        // std::map<_I,_I> ref_add_map( refs_vec[rr].get_Vector_I("Map") );
        // std::map<_I,_I> ref_add_map( refs_vec[rr].get_Add_Map("Map") );
        // std::map<_I,_I> ref_add_map = refs_vec[rr].get_Add_Map("Map");
        
        idat::Idat cur_ref_idat( refs_vec[rr] );
        
        // Calculate Tro:
        _I cur_cnt = 0;
        // _d cur_sr2 = 0.0;
        std::vector< _d > own_int_vec;
        std::vector< _d > ref_int_vec;
        for ( std::size_t ii=0; ii<Address_Cnt; ii++ ) {
          // _I can_add = Address_Idx[ii];
          // _I ref_add = refs_vec[rr].Address_Idx[ii];
          // if ( ref_add_map.find( can_add ) != ref_add_map.end() ) cur_cnt++;
          // const _Y is_found = refs_vec[rr].contains_address(can_add);
          
          // const _Y is_found = cur_ref_idat.contains_address(Address_Idx[ii]);
          // if ( is_found ) cur_cnt++;
          
          if ( cur_ref_idat.contains_address(Address_Idx[ii]) ) {
            cur_cnt++;
            
            // Need to push intensities of to this/ref stack for r2/dB calcs
            own_int_vec.push_back( (_d)Address_Avg[ii] );
            ref_int_vec.push_back( (_d)cur_ref_idat.get_avg_add(Address_Idx[ii]) );
          }
        }
        // std::cerr <<"ints[0]="<<own_int_vec[0]<<","<<ref_int_vec[0]<<std::endl;
        // std::cerr <<"ints[1]="<<own_int_vec[1]<<","<<ref_int_vec[1]<<std::endl;
        
        _d cur_sr2 = pearsoncoeff_vec( own_int_vec,ref_int_vec );
        
        // [TBD]: Fix the logic to update base on cur_cnt -> manifest; then cur_sr2 -> sample
        if ( cur_cnt >= top_cnt && cur_sr2 > top_sr2 ) {
          top_cnt = cur_cnt;
          top_idx = rr;
          top_sr2 = cur_sr2;
        }
        if ( p4 ) std::cerr<<_fo<<"Current Count["<<rr<<"]: "<<cur_cnt<<std::endl;
      }
      
      //
      // LEFT OFF HERE:: 
      // [TBD]: Update the this->cmp_vec and other params...
      //
      // Report the top match::
      idat::Idat top_idat(refs_vec[top_idx]);
      idat_cmp_struct top_cmp( this->get_cmp_idx( top_idat, 0 ) );
      top_cmp.Tro = int( 100 * top_cnt / Address_Cnt );
      top_cmp.Sr2 = (_d) top_sr2;
      
      // top_cmp.Tro = 2; // Just a check...
      this->cmp_vec.push_back( top_cmp );
      
      if ( p4 ) std::cerr
      <<_fo<<"Top Match["<<top_idx<<"]: "<<top_cnt<<"\n"
      <<top_idat.to_string()
      <<std::endl;
      
    } else {
      // 3.a Initialize PnegECDF Background Signal Data
      calc_pval = pval_vec.size() != 0;
      if ( calc_pval ) {
        for ( auto& add : pval_vec ) pval_map[ add ] = 0;
        for ( std::size_t ii=0; ii<Address_Cnt; ii++ ) {
    ...
    if ( calc_pval ) this->vd_dat[ "Det" ] = Address_Det;
  ...
  // Get Avg Intensity of an address
  _H get_avg_add( _I add ) {
    if ( this->add_map["Map"].find( add ) != this->add_map["Map"].end() )
      return( this->vH_dat["Sig"][ this->add_map["Map"][add] ] );
    
    // Should return something smater...
    return(0);
  }
  
  // [TBD]: Boolean Is Address Present::
  const _Y contains_address( _I add ) { 
    if ( this->add_map["Map"].find( add ) != this->add_map["Map"].end() ) return(true);
    return( false );
  };
  //
  // [TBD]: NOT SURE THIS WORKS:: Get Address Map:: Address_Map
  std::map<_I,_I> get_Add_Map( _S field ) { return( this->add_map[ field ] ); };
  
  // Get cmp_vec[idx]
  // [TBD]: Validate idx is valid otherwise return null
  idat_cmp_struct get_cmp_idx( Idat x, _I idx ) { return( x.cmp_vec[idx] ); };
  ...
  // [NOTE]: the record keeping method is currently not used...
  void add_cmps( _S Man,
               _S Sam,
               _d Tro,
               _d Sr2,
               _d SdB )
  {
    idat_cmp_struct cur_cmp;
    ...
    this->cmp_vec.push_back( cur_cmp );
  }
                ...
                const _S ft="to_string" )
  {
    ...
    if ( format.compare("tab") == 0 )
    {
      ...
      // Print Ordered Comparison Vector::
      for ( auto& it : cmp_vec )
        ss<<prefix<<"Cmp = '"<<it.Man<<","
          <<it.Sam<<","<<it.Tro<<","<<it.Sr2<<","<<it.SdB
          <<"'\n"<<std::endl;
      // ss<<prefix<<it.Man<<" = '"<<stringify_sum(it.second)<<"'"<<std::endl;
      // ss<<prefix<<it.first<<" = '"<<stringify_sum(it.second)<<"'"<<std::endl;
