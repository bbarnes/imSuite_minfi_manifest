...
// [[Rcpp::export]]
Rcpp::DataFrame seqs_to_2bit_rcpp( std::vector< _S >& seq_vec,
                                   ...
                                   const unsigned int vt = 1,
                                   const unsigned int tc = 0,
                                   const std::string ft="seqs_to_2bit_rcpp" )
{
  const std::string tb(tc, '\t');
  const std::string _fo("["+ft+"]: "+tb);
  const std::string _fe("["+ft+"]: ERROR: ");
...
// [[Rcpp::export]]
Rcpp::DataFrame fwd2tops_rcpp( Rcpp::StringVector  seq_vec_r,
                               Nullable<Rcpp::StringVector>  cgn_vec_r_ = R_NilValue,
                               Nullable<Rcpp::IntegerVector> pos_vec_r_ = R_NilValue,
                               
                               const std::string cgn_key = "Probe_ID",
                               const int var_len = 2,
                               const bool add_fwd = false,
                               const bool add_top = false,
                               const bool add_tbs = false,
                               const bool add_grp = false,
                               const bool add_ord = false,
                               const bool uc = false,
                               const unsigned int max = 0,
                               const unsigned int vb = 0,
                               const unsigned int vt = 1,
                               const unsigned int tc = 0,
                               const std::string ft="fwd2tops_rcpp" )
{
  const std::string tb(tc, '\t');
  const std::string _fo("["+ft+"]: "+tb);
  const std::string _fe("["+ft+"]: ERROR: ");
  ...
  // Resize if max is provided::
  unsigned int seq_vec_size = seq_vec_r.size();
  if ( max > 0 && max < seq_vec_size ) seq_vec_r.erase( max, seq_vec_size );
  seq_vec_size = seq_vec_r.size();
  ...
  if ( p0 )
    ...
    std::vector<unsigned int> grp_vec( seq_vec_size * 2 );
    std::vector<unsigned int> ord_vec( seq_vec_size * 2 );
    std::vector<unsigned int> pos_vec( seq_vec_size * 2 );
    std::vector<std::string>  cgn_vec( seq_vec_size * 2 );
    
    // [TBD]: Reduce Pos/CGN complexity
    // [TBD]: Reduce Verbose Messaging
    // [TBD]: Add ord_vec to output
    // [TBD]: Reduce output option complexity
    
    // Initialize CGN if present::
    Rcpp::StringVector cgn_vec_r;
    if ( cgn_vec_r_.isNotNull() ) cgn_vec_r = cgn_vec_r_;
    unsigned int cgn_vec_size = cgn_vec_r.size();
    
    // Initialize Pos if present::
    Rcpp::IntegerVector pos_vec_r;
    if ( pos_vec_r_.isNotNull() ) pos_vec_r = pos_vec_r_;
    unsigned int pos_vec_size = pos_vec_r.size();
    ...
    for ( unsigned int seq_idx = 0; seq_idx < seq_vec_size; seq_idx++ ) {
      seq_str = seq_vec_r( seq_idx );
      if ( seq_idx < cgn_vec_size ) cgn_str = cgn_vec_r( seq_idx );
      if ( seq_idx < pos_vec_size ) pos_int = pos_vec_r( seq_idx );
      
      std::vector<std::string> cur_vec = expand_iupac_seq( seq_str, vb, vt+1 );
      ...
      for ( unsigned int exp_idx = 0; exp_idx < cur_vec_size; exp_idx++ )
      {
        ...
        if ( new_idx + 2 > exp_vec_size )
        {
          ...
          grp_vec.resize( new_vec_size );
          ord_vec.resize( new_vec_size );
          if ( seq_idx < pos_vec_size ) pos_vec.resize( new_vec_size );
          if ( seq_idx < cgn_vec_size ) cgn_vec.resize( new_vec_size );
        }
        
        // Assign new expanded sequences and coordinates::
        exp_vec[ new_idx ] = cur_vec[exp_idx];
        fwd_vec[ new_idx ] = seq_str;
        grp_vec[ new_idx ] = seq_idx;
        ord_vec[ new_idx ] = exp_idx;
        if ( seq_idx < pos_vec_size ) pos_vec[ new_idx ] = pos_vec_r( seq_idx );
        if ( seq_idx < cgn_vec_size ) cgn_vec[ new_idx ] = cgn_vec_r( seq_idx );
        exp_raw_cnts++;
        exp_last_idx++;
    ...
    grp_vec.resize( exp_last_idx );
    if ( pos_vec_size > 0 ) pos_vec.resize( exp_last_idx );
    if ( cgn_vec_size > 0 ) cgn_vec.resize( exp_last_idx );
    
    unsigned int new_exp_size = exp_vec.size();
    unsigned int new_fwd_size = fwd_vec.size();
    unsigned int new_pos_size = pos_vec.size();
    unsigned int new_grp_size = grp_vec.size();
    ...
    if ( cgn_vec_size > 0 ) ret_df.push_back( cgn_vec, cgn_key );
    if ( pos_vec_size > 0 ) ret_df.push_back( pos_vec, "Cpg_Position" );
    ret_df.push_back( exp_vec, "Exp_Sequence" );
    if ( add_fwd ) ret_df.push_back( fwd_vec, "Fwd_Sequence" );
    if ( add_top ) ret_df.push_back( top_vec, "Top_Sequence" );
    if ( add_tbs ) ret_df.push_back( tbs_vec, "Strand_TB" );
    if ( add_grp ) ret_df.push_back( grp_vec, "Group_Idx" );
    if ( add_ord ) ret_df.push_back( ord_vec, "Expand_Idx" );
    
    // type.convert( x, na.strings = "NA", as.is, ... )
    return( typeConvert( asTibble( ret_df ), "", true ) );
}
...
StringVector expand_seqs_cpp( Rcpp::StringVector seqs,
  ...
  Rcpp::Rcerr << "This function is deprecated, use fwd2tops_rcpp()...\n" << std::endl;
  StringVector rt( 0 );
  return(rt);
...
if ( run_2bit_test ) {
  ...
  seq_vec <- NULL
  seq_vec <- c(
    paste( rep( "A", 48 ), collapse = "" ),
    paste( rep( "C", 48 ), collapse = "" ),
    paste( rep( "G", 48 ), collapse = "" ),
    paste( rep( "T", 48 ), collapse = "" )
  )
