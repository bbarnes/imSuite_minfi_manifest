                             ...
                             fun_tag='man_to_prb_stack')
{
  ...
  tib <- tib %>% dplyr::select( dplyr::any_of( c(cgn_vec,prb_vec,pos_vec) ) )
  
  if ( !is.null( cgn_vec ) ) tib <- tib %>% 
    tidyr::unite( Probe_ID, dplyr::any_of(cgn_vec), sep="_")
  
  if ( !is.null( pos_vec ) ) tib <- tib %>% 
    tidyr::unite( Coordinate, dplyr::any_of(pos_vec), sep="_") %>%
    type.convert( na.strings = "", as.is=TRUE)
  
  for ( prb_idx in c(1:length(prb_vec)) ) {
    prb_str = prb_vec[prb_idx]

    ret_tib <- ret_tib %>% dplyr::bind_rows(
      tib %>% 
        dplyr::rename( Probe_Seq = !!prb_str ) %>%
        dplyr::filter( !is.na(Probe_Seq) ) %>%
        dplyr::select( dplyr::any_of( c("Probe_ID","Probe_Seq","Coordinate") ) ) %>%
        dplyr::mutate( Probe_Idx = prb_idx )
    )
  }
  
  # Join Id and Idx::
  if ( !is.null( cgn_vec ) ) ret_tib <- ret_tib %>% 
    tidyr::unite( Probe_ID, c("Probe_ID","Probe_Idx"), sep="_" )
  
  if ( sort_prb ) ret_tib <- ret_tib %>% dplyr::arrange( Probe_Seq )
  if ( sort_cgn && !is.null( cgn_vec ) ) ret_tib <- ret_tib %>% 
    dplyr::arrange( Probe_ID )
  
  if ( expand_prb ) {
    cgn_dat <- NULL
    pos_dat <- NULL
    if ( !is.null( cgn_vec ) ) cgn_dat <- ret_tib$Probe_ID
    if ( !is.null( pos_vec ) ) pos_dat <- ret_tib$Coordinate
    
    ret_tib <- fwd2tops_rcpp( 
      seq_vec_r  = ret_tib$Probe_Seq,
      cgn_vec_r_ = cgn_dat, 
      pos_vec_r  = pos_dat %>% as.integer(),
      add_fwd = add_fwd, 
      add_top = add_top, 
      add_tbs = add_tbs, 
      add_grp = add_grp, 
    ...
    if ( !is.null( cgn_vec ) ) ret_tib <- ret_tib %>% 
      dplyr::distinct( Probe_ID,Exp_Sequence, .keep_all = TRUE ) %>%
      dplyr::select( Probe_ID,Exp_Sequence, dplyr::everything() )
  }
