...
# ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
#
#                     Sesame SDF Conversion Functions::
#
# ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #

format_rcpp_sdf = function( sdf_tib,
                            ...
                            fun_tag='format_rcpp_sdf' )
{
  
  Mask_sym   <- rlang::sym( paste0("mask_",stat_idx) )
  Pval_sym   <- rlang::sym( paste0("Pval_",stat_idx) )
  UG_Det_sym <- rlang::sym( paste0("UG_Det_",stat_idx) )
  UR_Det_sym <- rlang::sym( paste0("UR_Det_",stat_idx) )
  ...
  # Just use predefined masking for c++ code...
  # 
  neg_tib <- NULL
  if ( add_negs ) {
    neg_tib <- sdf_tib %>%
      ...
      dplyr::filter( !!Mask_sym == TRUE ) %>%
      dplyr::mutate( mask = TRUE ) %>%
    ...
    cgn_tib <- sdf_tib %>%
      ...
      dplyr::mutate(
        mask = dplyr::case_when(
          # !!Pval_sym <= cgn_pval ~ FALSE,
          # TRUE ~ TRUE
          !!Pval_sym > cgn_pval ~ TRUE,
          TRUE ~ FALSE
        )
      )
  }
  ...
  ret_tib <- NULL
  ret_tib <- dplyr::bind_rows( cgn_tib,neg_tib,ctl_tib ) %>%
...
format_rcpp_sdf3 = function( sdf_tib,
                             ...
                             fun_tag='format_rcpp_sdf3' )
{
  ...
  # This needs to be better understood for negative control screening...
  # 
  neg_pval <- 1.0 - neg_pval
  # print(neg_pval)
  neg_tib <- NULL
  if ( add_negs ) {
    neg_tib <- sdf_tib %>%
      dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ct") ) %>%
      dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>%
      # dplyr::filter( pmin( !!UG_Det_sym,!!UR_Det_sym) < neg_pval ) %>%
      dplyr::filter( 1 - !!UG_Det_sym <= neg_pval ) %>%
      dplyr::filter( 1 - !!UR_Det_sym <= neg_pval ) %>%
      dplyr::mutate( mask = TRUE ) %>%
      dplyr::distinct( Probe_ID, .keep_all = TRUE )
  }
...
# pre_sdf_tib %>% dplyr::filter( Probe_ID %>% stringr::str_starts("ctl_") ) %>% dplyr::filter( !Probe_ID %>% stringr::str_starts("ctl_Negative") ) %>% dplyr::arrange( Probe_ID ) %>% dplyr::pull( Probe_ID ) %>% stringr::str_remove("^ctl_") %>% stringr::str_remove("-[0-9]+$") %>% unique()
# sdf_tib = pre_sdf_tib
# ctl_tib = msa_man_tib
#
# exp_ctl_tib
# tmp_ctl_dat <- sdf_to_sesame_ctls( sdf_tib = pre_sdf_tib, ctl_tib = msa_man_tib, min_pval = 0.05, neg_pval = 0.00, ret_data = TRUE )
sdf_to_sesame_ctls = function( sdf_tib,
                               ctl_tib,
                               
                               sentrix_name = "chip_UNK",
                               outliers_str = "na-outliers",
                               ctl_pval = 0.05,
                               neg_pval = 0.00,
                               stat_vec = c(0,1),
                               ...
                               fun_tag='sdf_to_sesame_ctls' )
{
  ...
  # Desired Output Format for Sesame::
  #
  # A tibble: 4,077 × 4
  # Address Type     Color_Channel Name       
  # <dbl> <chr>    <chr>         <chr>      
  # 1 71774287 NEGATIVE Red           Negative_1 
  # 2 68760974 NEGATIVE Red           Negative_2 
  ...
  for (stat_idx in stat_vec ) {
    neg_tib <- NULL
    alt_tib <- NULL
    if ( stat_idx == 0 ) {
      neg_tib <- sdf_tib %>%
        dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>%
        dplyr::filter( UG_Det_0 >= neg_pval & UR_Det_0 >= neg_pval )
      alt_tib <- sdf_tib %>%
        dplyr::filter( !stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>%
        dplyr::filter( Pval_0 <= ctl_pval )
    } else if ( stat_idx == 1 ) {
      neg_tib <- sdf_tib %>%
        dplyr::filter( stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>%
        dplyr::filter( UG_Det_1 >= neg_pval & UR_Det_1 >= neg_pval )
      alt_tib <- sdf_tib %>%
        dplyr::filter( !stringr::str_starts( Probe_ID, pattern = "ctl_Negative") ) %>%
        dplyr::filter( Pval_1 <= ctl_pval )
    } else {
      errs_mssg <- glue::glue("File file='{file}' does not exist")
      stop(glue::glue("{errs} {errs_mssg}!{errs} Exiting...{RET2}"))
      return(NULL)
    }
    
    sdf_tib <- dplyr::bind_rows( neg_tib,alt_tib ) %>%
      dplyr::arrange( Probe_ID )
    
    ctl_tib <- ctl_tib %>% dplyr::filter( Probe_ID %in% sdf_tib$Probe_ID ) %>%
      dplyr::arrange( Probe_ID ) %>%
      dplyr::distinct( U, .keep_all = TRUE )
    
    cur_tib <- NULL
    cur_tib <- dplyr::bind_rows( 
      # dplyr::select( ctl_tib,U,Annotation,col,Probe_ID ) %>% 
      dplyr::select( ctl_tib,U,Probe_Type,col,Probe_ID ) %>% 
        # dplyr::rename( Address=U, Type=Annotation, Color_Channel=col, Name=Probe_ID ) %>%
        dplyr::rename( Address=U, Type=Probe_Type, Color_Channel=col, Name=Probe_ID ) %>%
        dplyr::filter( Address != 0 ) %>%
        dplyr::mutate( Name = paste0(Name,"_U") ),
      # dplyr::select( ctl_tib,M,Annotation,col,Probe_ID ) %>% 
      dplyr::select( ctl_tib,M,Probe_Type,col,Probe_ID ) %>% 
        # dplyr::rename( Address=M, Type=Annotation, Color_Channel=col, Name=Probe_ID ) %>%
        dplyr::rename( Address=M, Type=Probe_Type, Color_Channel=col, Name=Probe_ID ) %>%
        dplyr::filter( Address != 0 ) %>%
        dplyr::mutate( Name = paste0(Name,"_M") ),
      NULL
    ) %>% 
      dplyr::arrange( Name ) %>%
    ...
    #
    # Get Negative Plot Stats::
    #
    cur_plot_tib <- NULL
    cur_plot_tib <- dplyr::bind_rows(
      dplyr::select( neg_tib,Probe_ID,UG,UG_Det_0,mask) %>% 
        dplyr::rename( Sig=UG,Pval=UG_Det_0) %>% dplyr::mutate(col="Grn"),
      dplyr::select( neg_tib,Probe_ID,UR,UR_Det_0,mask) %>% 
        dplyr::rename( Sig=UR,Pval=UR_Det_0) %>% dplyr::mutate(col="Red"),
      NULL
    ) %>% dplyr::arrange( Probe_ID ) %>%
      dplyr::mutate( Stat_Idx = stat_idx )
    
    neg_plot_tib <- dplyr::bind_rows( neg_plot_tib,cur_plot_tib )
    
    cur_sum <- NULL
    cur_sum <- neg_plot_tib %>% 
      # dplyr::filter( Pval > 0.90 ) %>%
      dplyr::group_by( mask,col ) %>%
      dplyr::summarise( 
        Tot_Cnt = n(),
        
        Sigs_Min=round( min( Sig,na.rm=TRUE), 1),
        Sigs_Avg=round( mean( Sig,na.rm=TRUE), 1),
        Sigs_Med=round( median( Sig,na.rm=TRUE), 1),
        Sigs_Max=round( max( Sig,na.rm=TRUE), 1),
        
        Pval_Min=round( min( Pval,na.rm=TRUE), 3),
        Pval_Avg=round( mean( Pval,na.rm=TRUE), 3),
        Pval_Med=round( median( Pval,na.rm=TRUE), 3),
        Pval_Max=round( max( Pval,na.rm=TRUE), 3),
      ...
      dplyr::select( Sentrix_Name,Rm_Outliers, 
                     Ctl_Pval,Neg_Pval,Stat_Idx, dplyr::everything() )
    if ( p1 ) cur_sum %>% print( n=base::nrow(cur_sum) )
    
    neg_sum <- neg_sum %>% dplyr::bind_rows( cur_sum )
  }
  
  if ( !is.null(out_path) ) {
    
    out_name <- NULL
    out_path <- safe_mkdir( out_path )
    out_name <- paste(sentrix_name,
                      outliers_str,
                      paste0("min-pval-",ctl_pval),
                      paste0("neg-pval-",neg_pval),
                      paste0("stat_idx-",stat_idx),
                      sep="_" )
    
    neg_csv <- file.path( out_path, paste0(out_name,".sum.csv.gz") )
    readr::write_csv( x = neg_sum, file = neg_csv )
    
    neg_den_plot_pdf <- file.path( out_path, paste0(out_name,".den.pdf") )
    neg_den_plot_ggg <- NULL
    neg_den_plot_ggg <- neg_plot_tib %>% 
      # dplyr::filter( Pval >= 0.00 ) %>%
      ggplot2::ggplot( aes( x=Sig, color=col) ) +
      ggplot2::geom_density() +
      ggplot2::facet_grid( rows = vars(Stat_Idx) ) +
      ggplot2::labs( title = paste0("Negative Controls: ",out_name) )
    
    ggplot2::ggsave( filename = neg_den_plot_pdf, plot = neg_den_plot_ggg, 
  ...
  if ( FALSE && ret_data ) {
    ret_dat <- NULL
    ret_dat$neg_tib <- neg_tib
    ret_dat$alt_tib <- alt_tib
    ret_dat$sdf_tib <- sdf_tib
    ret_dat$ctl_tib <- ctl_tib
