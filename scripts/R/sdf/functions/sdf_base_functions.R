...
# From Sesame:: Preprocessing Function Code: prepSesameList()
#
...
# I	ELBAR	Mask background-dominated readings
# B	noob	Background subtraction using oob
# S	inferSpecies	Set species-specific mask
# T	inferStrain	Set strain-specific mask (mouse)
# M	matchDesign	Match Inf-I/II in beta distribution
...
# [TBD]: Add formatVCF() as an option code...
#
                              ...
                              fun_tag='mutate_sdf_simple')
{
  ...
  for ( s in step_vec ) {
    ...
    if ( s == "N" ) sdf = sesame::detectionPnegEcdf(     sdf = sdf, return.pval = FALSE, pval.threshold = negs_min )
    if ( s == "n" ) sdf = sesame::detectionPnegEcdf(     sdf = sdf, return.pval = TRUE,  pval.threshold = negs_min )
  ...
  # prb_sym = rlang::sym( probe_id )
  # val_sym = rlang::sym( beta_key )
  if ( ret_tib &&
       (s == "V" || s == "v" || s == "W" || s == "w" ||
        s == "O" || s == "o" ||
        s == "n" ) ) sdf <- sdf %>% 
    ...
    tibble::as_tibble() %>% 
    magrittr::set_names( value = c(all_of(c(probe_id,beta_key))) )
