...
# print %bases2iupac."\n";
if (FALSE) {
    foreach my $base (sort keys %bases2iupac) {
	print "$base\t$bases2iupac{$base}\n";
    }
    print "\n";
}
...
my @IUPAC_ONLY = ($R, $Y, $S, $W, $K, $M, $B, $D, $H, $V, $N);

# iupacNucs
my %codeLines = ();
foreach my $nucA (sort keys %iupacNucs) {
    foreach my $nucB (sort @{$iupacNucs{$nucA}}) {
	my $code1 = "MAP[['".uc($nucA).uc($nucB)."']] <- TRUE";
	my $code2 = "MAP[['".uc($nucB).uc($nucA)."']] <- TRUE";
	my $code3 = "MAP[['".uc($nucA).uc($nucA)."']] <- TRUE";
	my $code4 = "MAP[['".uc($nucB).uc($nucB)."']] <- TRUE";
...
my $nucA=$N;
foreach my $nucB (sort @IUPAC_ONLY) {
    my $code1 = "MAP[['".uc($nucA).uc($nucB)."']] <- TRUE";
    my $code2 = "MAP[['".uc($nucB).uc($nucA)."']] <- TRUE";
    my $code3 = "MAP[['".uc($nucA).uc($nucA)."']] <- TRUE";
    my $code4 = "MAP[['".uc($nucB).uc($nucB)."']] <- TRUE";
...
my $nucB=$N;
foreach my $nucA (sort @IUPAC_ONLY) {
    my $code1 = "MAP[['".uc($nucA).uc($nucB)."']] <- TRUE";
    my $code2 = "MAP[['".uc($nucB).uc($nucA)."']] <- TRUE";
    my $code3 = "MAP[['".uc($nucA).uc($nucA)."']] <- TRUE";
    my $code4 = "MAP[['".uc($nucB).uc($nucB)."']] <- TRUE";
...
foreach my $line (sort keys %codeLines) {
    # print "$codeLines{$line}\n";
    print "$line\n";
}
