...
while(<$datFH>) {
    ...
    next if (defined($FR) && defined($data[21]) && $data[21] ne $FR);
    next if (defined($TB) && defined($data[22]) && $data[22] ne substr($TB, 1,1));
    next if (defined($CO) && defined($data[23]) && $data[23] ne $CO);
