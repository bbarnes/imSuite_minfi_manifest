...
foreach my $canVcfFile (@{$options{'can-files'}}) {
    ...
    ## Set up the additional INFO fields for the reference headers
    my $refHeader = "\#\#INFO=<ID=MATCHSRC,Number=1,Type=Integer,Description=" .
	"\"This describes the source file matched against. Definitions follow: 1 = " .
	"$canVcfFile\">\n";
    $refHeader .= "\#\#INFO=<ID=MATCHTYPE,Number=1,Type=Float,Description=" .
	"\"This describes the best match found. Definition follows: 1 = >90\% reciprocal " .
	"overlap and same type, 2 = <90\% reciprocal overlap and same type, 3 = breakpoint match type, " .
	"4 = mismatched type, 5 = no match.\">\n";
    $refHeader .= "\#\#INFO=<ID=MATCHIDX,Number=1,Type=Integer,Description=" .
	"\"This describes the index of the best match found.\">\n";

    ## The candidate is a little different because there can be multiple reference files
    my $canHeader = "";
	...
	##
	## We should have the candidate calls and the current reference
	##  calls in memory now. So we can process the associated
	##  intersection file on the fly. 
	##
	open(INTER, "<$options{'int-files'}[$intIdx]")
	    or die("Failed to open $options{'int-files'}[$intIdx] for reading: $!");
	mssg("\# Loading intersection: $options{'int-files'}[$intIdx]\n");
	...
	while(<INTER>) {
	    ...
	    my $call = getCall(\@fields);
	    
	    ## Update candidate call table
	    push @{$canCalls[$intCanIdx][$cIdx]{$call}}, [ $intRefIdx, $rIdx ];
	    
	    ## Update refernce call table
	    push @{$refCalls[$rIdx]{$call}}, $cIdx;
	}
	close(INTER);
	$intIdx++;
	...
	##
	my %refCallStats = ();
	my $refOutFile = "$options{'out-dir'}/ref-$intRefIdx.vcf";
	open(REFOUT, ">$refOutFile")
	    or die("Failed to open $refOutFile for writing: $!");
