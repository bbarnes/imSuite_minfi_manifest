...
while(<INT>) {
    ...
    ##
    ## Here we actually need to handle multiple mappings
    ##

    push @{$mapA{$keyA}}, [ $idxB, $sideA, $sideB, $min, $max, $idB ];
    push @{$mapB{$keyB}}, [ $idxA, $sideA, $sideB, $min, $max, $idA ];
...
for (my $ii = 0; $ii < @bedDat1; $ii++) {
    ...
    if (defined($mapA{$idx})) {
	for (my $jj = 0; $jj < @{$mapA{$idx}}; $jj++) {
	    my @mapDat = @{$mapA{$idx}[$jj]};
