...
my (%drmM, %mapM, %mapD, %bspFR, %bspCO, %bspTB);
...
sub buildProbes {
    my $func=(caller(0))[3];
    my ($prbREF, $fwdSeq, $hacSeq, $prbCG, $prbIT, $prbCO, $prbFR, $prbTB, $isBSC) = @_;
    ...
    # Bisulfite/Dream Sample Prep::
    foreach my $mu (@bscMUD) {
	$refSeqs{$F}{$C}{$mu} = bsc($refSeqs{$F}{$C}{$N}, $mu, $isBSC);
	$refSeqs{$F}{$O}{$mu} = revCmpl($refSeqs{$F}{$C}{$mu});
	$refSeqs{$R}{$C}{$mu} = bsc($refSeqs{$R}{$C}{$N}, $mu, $isBSC);
	$refSeqs{$R}{$O}{$mu} = revCmpl($refSeqs{$R}{$C}{$mu});
	    
	$snvSeqs{$F}{$C}{$mu} = bsc($snvSeqs{$F}{$C}{$N}, $mu, $isBSC);
	$snvSeqs{$F}{$O}{$mu} = revCmpl($snvSeqs{$F}{$C}{$mu});
	$snvSeqs{$R}{$C}{$mu} = bsc($snvSeqs{$R}{$C}{$N}, $mu, $isBSC);
...
sub loadManifest {
    my $func=(caller(0))[3];
    my ($inpFile,$srcType, $ordFH, $annFH, $excFH, $incFH, $drmFH, $alnFH, $cntFH)=@_;

    die("\n[$func]: ERROR: Function call with missing paramaters (designFile, sourceType [manifest|design|design+], ordFH, annFH, excFH)\n")
	if (! defined($inpFile) || ! defined($srcType) || ! defined($ordFH) ||
	    !defined($annFH) || !defined($excFH) || !defined($drmFH) || !defined($cntFH) );
    die("\n[$func]: ERROR: Fasta File does not exist: '$inpFile'!\n") if (! -e $inpFile);
	...
	my @data;
	my ($desCG, $desIT, $desIt, $desFR, $desCO, $desTB, $desNB,
	    $prbSeqA, $prbSeqB, $fwdSeq, $hacSeq, $repNM,
	    $chr, $pos);
	...
	if ($srcType eq 'manifest') {
	    ...
	    $pos = $data[$colToIdx{'MAPINFO'}];

	    next if ( $chr eq "0" || $pos eq "0" );

	    $desCG = $data[$colToIdx{'IlmnID'}];
	    # [NOTE]: Skipping weird probes from EX100k::
	    next if ( $desCG eq "cg02774963_TC21" );
	    next if ( $desCG eq "cg17220237_TC21" );
	    next if ( $desCG eq "cg23406436_BC21" );

	    # Skip possible multi-uniuq probes at the end of the file...
	    # [NOTE]: This doesn't work if we sort by genomic coordinates...
	    # last if ( $desCG eq "cg00241076_TC11" );
	    # These are probabaly multi-unique probes::
	    next if ( $desCG eq "cg00241076_TC11" );
	    next if ( $desCG eq "cg03013259_TC11" );
	    next if ( $desCG eq "cg12922415_BC12" );	    
	    next if ( $desCG eq "cg13220870_BC11" );
	    next if ( $desCG eq "cg23951171_TC11" );
	    next if ( $desCG eq "cg24717141_TC12" );

	    next if ( $desCG eq "cg13789775_TC21" );
	    next if ( $desCG eq "cg10111988_BC21" );
	    # next if ( $desCG eq "" );

	    # [NOTE]: Will use Name instead of IlmnID since we can combine the extra info later...
	    $desCG = $data[$colToIdx{'Name'}];
	    $desIT = $data[$colToIdx{'Infinium_Design_Type'}];
	    $desFR = $data[$colToIdx{'Strand_FR'}];
	    $desCO = $data[$colToIdx{'Strand_CO'}];
	    $desTB = $data[$colToIdx{'Strand_TB'}];
	    $desNB = $data[$colToIdx{'Next_Base'}];

	    #
	    # [NOTE]: Skipping Rep_Num > 1
	    #
	    $repNM = $data[$colToIdx{'Rep_Num'}];
	    next if ($repNM > 1);
	    ...
	    # Skip weird sequences::
	    next if ( $fwdSeq =~ m/\// && $desCG =~ m/^cg/ );

	    # Fix SNP Forward Sequences::
	    if ( $desCG =~ m/^rs/ ) {
		# $fwdSeq =~ s/\[.*\]/[CG]/;
		$fwdSeq =~ s/\///;
		$hacSeq =~ s/\///;
	    ...
	    #
	    # [NOTE]: For some reason new manifests have the strand flipped???
	    #
	    if ( $desFR eq "F" ) {
		$desFR = "R";
	    } elsif ( $desFR eq "R") {
		...
		die("\n[$func]: ERROR: Invalid Strand_FR='$desFR'.\n\n");
	    }

	    #
	    # [NOTE]: Skipping Opposite Designs for now...
	    #
	    next if ( $desCO eq "O" );
	...
	$desIt = 1 if ($desIT eq  'I');
	$desIt = 2 if ($desIT eq 'II');
	$infTypes{$desIT}++;
	...
	my ($preSeq, $midSeq, $nxtNuc, $sufSeq);
	if ($prbTP eq "rs" && $fwdSeq =~ m/^([ACTG]+)\[([A-Z][A-Z]+)\]([A-Z])([ACTG]+)$/) {
	    $preSeq=$1;
	    $midSeq=$2;
	    $nxtNuc=$3;
	    $sufSeq=$4;
	    $fwdSeq = $preSeq."[".$bases2iupac{$midSeq}.$nxtNuc."]".$sufSeq."N";
	    $hacSeq = $fwdSeq;
	} elsif ($prbTP eq "cg" && $fwdSeq =~ m/^(N*[ACTG]+)\[(CG)\]([ACTG]+N*)$/) {
	    $preSeq=$1;
	    $midSeq=$2;
	    $sufSeq=$3;
	...
	my @prbs=(); # Bisulfite Converted (Target Infinium Design)
	
	my @bsc1=(); # Bisulfite Converted (Infinium I)
	my @bsc2=(); # Bisulfite Converted (Infinium II)
	
	my @drm1=(); # For Dream Design (Infinium I)
	my @drm2=(); # For Dream Design (Infinium II)
	...
	#
	my $drmCG  = join($L, $desCG, join("", $desTB, $desCO) );
	my $drmCG1 = join($L, join("", $drmCG,"1",$repNM), $desFR,"drm");
	my $drmCG2 = join($L, join("", $drmCG,"2",$repNM), $desFR,"drm");
	
	my $drmCG_D=join($L, $drmCG2, $A);
	my $drmCG_A=join($L, $drmCG1, $A);
	my $drmCG_B=join($L, $drmCG1, $B);

	my $bscCG  = join($L, $desCG, join("", $desTB, $desCO) );
	my $bscCG1 = join($L, join("", $bscCG,"1",$repNM), $desFR,"bsc");
	my $bscCG2 = join($L, join("", $bscCG,"2",$repNM), $desFR,"bsc");
	
	my $bscCG_D=join($L, $bscCG2, $A);
	my $bscCG_A=join($L, $bscCG1, $A);
	my $bscCG_B=join($L, $bscCG1, $B);

	# Standard Ordering::
	$desCG = join($L, $desCG, join("", $desTB,$desCO,$desIt,$repNM), $desFR,"bsc") if ($srcType eq 'manifest');
	my $desCG_A=join($L, $desCG, $A);
	my $desCG_B=join($L, $desCG, $B);

	#
	# [TBD]: Copy buildProbes, but return D,U,M and number of CpG Sites...
	#        - Actually just call it twice...
	buildProbes(\@prbs, $fwdSeq, $hacSeq, $desCG, $desIT, $desCO, $desFR, $desTB, TRUE);

	buildProbes(\@bsc1, $fwdSeq, $hacSeq, $desCG,    "I", $desCO, $desFR, $desTB, TRUE);
	buildProbes(\@bsc2, $fwdSeq, $hacSeq, $desCG,   "II", $desCO, $desFR, $desTB, TRUE);
	
	buildProbes(\@drm1, $fwdSeq, $hacSeq, $desCG,    "I", $desCO, $desFR, $desTB, FALSE);
	buildProbes(\@drm2, $fwdSeq, $hacSeq, $desCG,   "II", $desCO, $desFR, $desTB, FALSE);
	...
	if ($srcType eq 'designW') {
		...
		if (substr(uc($prbSeqA),1) ne substr(uc($desSeqA),1)) {
		    my $subA1 = substr(uc($prbSeqA),1);
		    my $subA2 = substr(uc($desSeqA),1);

		    print STDERR "$curCnt: $desCG\t$desIT\tOrgA\t$prbSeqA\t$subA1\n";
		    print STDERR "$curCnt: $desCG\t$desIT\tOrgB\t$prbSeqB\n";
		    print STDERR "$curCnt: $desCG\t$desIT\tDesA\t$desSeqA\t$subA2\n";
		    print STDERR "$curCnt: $desCG\t$desIT\tDesB\t$desSeqB\n";
		    print STDERR "\n";
		    die("\n[$func]: ERROR: Failed[desIT=$desIT,1] on line='$line'\n\n");
		} elsif ($desIT eq "I" && substr(uc($prbSeqB),1) ne substr(uc($desSeqB),1)) {
		    my $subB1 = substr(uc($prbSeqB),1);
		    my $subB2 = substr(uc($desSeqB),1);

		    print STDERR "$curCnt: $desCG\t$desIT\tOrgA\t$prbSeqA\n";
		    print STDERR "$curCnt: $desCG\t$desIT\tOrgB\t$prbSeqB\t$subB1\n";
		    print STDERR "$curCnt: $desCG\t$desIT\tDesA\t$desSeqA\n";
		    print STDERR "$curCnt: $desCG\t$desIT\tDesB\t$desSeqB\t$subB2\n";
		    print STDERR "\n";
		    die("\n[$func]: ERROR: Failed[desIT=$desIT,2] on line='$line'\n\n");
		} else {
		    $matCnt++;
		}
	    } elsif ($desIT eq $II) {
		...
		if ($prbSeqD ne substr(uc($desSeqD),1)) {
		    ...
		    print STDERR "\n";
		    die("\n[$func]: ERROR: Failed[desIT=$desIT,3] on line='$line'\n\n");
		} else {
		    $matCnt++;
		}
	    } else {
		...
		print STDERR "$curCnt: $desCG\t$desIT\tDesB\t$desSeqB\n";
		print STDERR "\n";
		print STDERR "$curCnt: $fwdSeq\n";
		print STDERR "\n";
		die("\n[$func]: ERROR: Failed[A] on line='$line'\n\n");
	    } elsif ($desIT eq "I" && substr(uc($prbSeqB),1) ne substr(uc($desSeqB),1)) {
		my $subB1 = substr(uc($prbSeqB),1);
		my $subB2 = substr(uc($desSeqB),1);
		...
		print STDERR "$curCnt: $desCG\t$desIT\tDesB\t$desSeqB\t$subB2\n";
		print STDERR "\n";
		die("\n[$func]: ERROR: Failed[B] on line='$line'\n\n");
	    } else {
		$matCnt++;
		my $desBD="mm10";
	...
	#
	# Write Order File:: Dream Converted::
	#
	print {$drmFH} join($COM, $drmCG2, $drmCG_D, $drmSeqD, '', '', "C")."\n";
	print {$drmFH} join($COM, $drmCG1, $drmCG_A, $drmSeqA, $drmCG_B, $drmSeqB, $binNB)."\n";

	#
	# Write Order File:: Bisulfite Converted::
	#
	if ($desIT eq $II) {
	...
	# [NOTE]: Double write Dream to Bisulfite Order file just for viewing...
	# print {$ordFH} join($COM, $drmCG2, $drmCG_D, $drmSeqD, '', '', "C")."\n";
	# print {$ordFH} join($COM, $drmCG1, $drmCG_A, $drmSeqA, $drmCG_B, $drmSeqB, $binNB)."\n";
	# [NOTE]: Actually going to write to the alignment file instead...

	my $matD = 0;
	my $matA = 0;
	my $matB = 0;

	$matD = 1 if ( uc($bscSeqD) eq uc($drmSeqD) );
	$matA = 1 if ( uc($bscSeqA) eq uc($drmSeqA) );
	$matB = 1 if ( uc($bscSeqB) eq uc($drmSeqB) );

	my $und_cpg_seq1 = substr($bscSeqD,1);
	$und_cpg_seq1 =~ s/[^RY]//gi;
	my $und_cpg_cnt1 = length($und_cpg_seq1);

	my $und_cpg_seq2 = substr($drmSeqD,1);
	$und_cpg_seq2 =~ s/[^RY]//gi;
	my $und_cpg_cnt2 = length($und_cpg_seq2);

	die("\n[$func]: ERROR: und_cpg counts don't match: $und_cpg_cnt1 != $und_cpg_cnt2\n\n")
	    if ( $und_cpg_cnt1 != $und_cpg_cnt2 );
	...
	my $datStr = join($COM, $desIt,$chr,$pos,$und_cpg_cnt1,$und_cpg_seq1);
	
	print {$alnFH} join($COM, $bscCG_D, $bscSeqD, "C",$matD,$datStr)."\n";
	print {$alnFH} join($COM, $drmCG_D, $drmSeqD, "C",$matD,$datStr)."\n";

	print {$alnFH} join($COM, $bscCG_A, $bscSeqA, $binNB,$matA,$datStr)."\n";
	print {$alnFH} join($COM, $drmCG_A, $drmSeqA, $binNB,$matA,$datStr)."\n";
	
	print {$alnFH} join($COM, $bscCG_B, $bscSeqB, $binNB,$matB,$datStr)."\n";
	print {$alnFH} join($COM, $drmCG_B, $drmSeqB, $binNB,$matB,$datStr)."\n";
	print {$alnFH} "\n";

	print {$cntFH} "$bscCG_D\t$und_cpg_cnt1\n";

	# [TBD]: Add Underlying CpGs
	# [TBD]: Sort by position
	# [TBD]: Add Must have aging probes
...
if (TRUE) {
    #
    # Quick Sorting Commands for EX100k -> Dream Designs
    #  gzip -dc /Users/bbarnes/mylocal/Documents/data/manifests/methylation/GenomeStudio/EX100K_GRCh37_1F.body-section.csv.gz | head -n 1 > /Users/bbarnes/mylocal/Documents/data/manifests/methylation/GenomeStudio/EX100K_GRCh37_1F.body-section.genomic-sorted.csv
    #  gzip -dc /Users/bbarnes/mylocal/Documents/data/manifests/methylation/GenomeStudio/EX100K_GRCh37_1F.body-section.csv.gz | grep -v "^Ilmn" | sort -n -k 17,17 -t, | sort -s -k 16,16 -t, >> /Users/bbarnes/mylocal/Documents/data/manifests/methylation/GenomeStudio/EX100K_GRCh37_1F.body-section.genomic-sorted.csv 
    #  gzip /Users/bbarnes/mylocal/Documents/data/manifests/methylation/GenomeStudio/EX100K_GRCh37_1F.body-section.genomic-sorted.csv
    #
    ...
    my ($ordFH, $drmFH, $annFH, $excFH, $incFH, $alnFH, $cntFH);
    $opts{'incFile'}=$opts{'outDir'}."/$species.order.included.csv.gz";
    $opts{'excFile'}=$opts{'outDir'}."/$species.order.excluded.csv.gz";
    $opts{'ordFile'}=$opts{'outDir'}."/$species.order.bsc.csv";
    $opts{'drmFile'}=$opts{'outDir'}."/$species.order.drm.csv";
    $opts{'annFile'}=$opts{'outDir'}."/$species.annotation.csv";
    $opts{'alnFile'}=$opts{'outDir'}."/$species.alignments.csv";
    $opts{'cntFile'}=$opts{'outDir'}."/$species.cpg-cnts.csv";
	...
	open($drmFH, ">$opts{'drmFile'}") or die("Failed to open $opts{'drmFile'}: for reading: $!");
	open($annFH, ">$opts{'annFile'}") or die("Failed to open $opts{'annFile'}: for reading: $!");
	open($alnFH, ">$opts{'alnFile'}") or die("Failed to open $opts{'alnFile'}: for reading: $!");
	open($cntFH, ">$opts{'cntFile'}") or die("Failed to open $opts{'cntFile'}: for reading: $!");
	...
	loadManifest($opts{'manFile'}, $opts{'srcType'}, $ordFH, $annFH, $excFH, $incFH, $drmFH, $alnFH, $cntFH);
	...
	# des2prbs($srd, $desSeq, $prbRef,$retUC)
	# des2prbs(0, $fwdSeq, \@prbs, 0);
	# buildProbes(\@prbs, $fwdSeq, $hacSeq, $desCG, $desIT, $desCO, $desFR);
	
	my $srd = 0;
	my @prbs1 = ();
	des2prbs($srd, $seqN, \@prbs1, 0);
	
	for ( my $ii=0; $ii<@{$prbs1[0][0]}; $ii++ ) {
	    mssg("[prbs=1, srd=$srd]: prbs1[0][0][$ii]='" . $prbs1[0][0][$ii] . "'\n");
	}
	...
	my @prbs2 = ();
	buildProbes(\@prbs2, $fwdSeq, $hacSeq, $desCG, $desIT, $desCO, $desFR, FALSE);
	
	for ( my $ii=0; $ii<@{$prbs2[0][0]}; $ii++ ) {
	    mssg("[prbs=2, srd=$srd]: prbs2[0][0][$ii]='" . $prbs2[0][0][$ii] . "'\n");
	}


	$des_cnt++;
	last if (defined($opts{'max'}) && $opts{'max'} != 0 && $opts{'max'} >= $des_cnt );
    }
}
...
sub bsc {
	...
	#
	if ($type eq $D) {
	    # D:  $seq =~ s/([CYSMBHV][GRSKBDV])/$mapD{$1}/g;
	    # D:  $seq =~ tr/CYSMBHV/ttkwkwd/;
	    #
	    return(bscNewD($seq));
	} elsif ($type eq $M) {
	    # M:  $seq =~ s/([CYSMBHV][GRSKBDV])/$mapM{$1}/g;
	    # M:  $seq =~ tr/CYSMBHV/ttkwkwd/;
	    #
	    return(bscNewM($seq));
	} elsif ($type eq $U) {
	    # U:  $seq =~ tr/CYSMBHV/ttkwkwd/;
	    #
	    return(bscNewU($seq));
	} else {
	...
	#
	if ($type eq $D) {
	    # D: $seq =~ s/([CYSMBHV][GRSKBDV])/$mapD{$1}/g; # [TBD]: Update %mapD{} to %drmD{} Not needed...
	    # Skip: $seq =~ tr/CYSMBHV/ttkwkwd/;
	    #
	    return(drmNewD($seq));
	} elsif ($type eq $M) {
	    # M: $seq =~ s/([CYSMBHV][GRSKBDV])/$mapM{$1}/g; [TBD]: Update %mapM{} to %drmM{}
	    # Skip: $seq =~ tr/CYSMBHV/ttkwkwd/;
	    #
	    return(drmNewM($seq));
	} elsif ($type eq $U) {
	    # U: Basically, do nothing...
	    #
	    # return(drmNewU($seq));
	    return($seq);
	} else {
...
sub drmNewM {
    ...
    die("[$func]: ERROR bscSeqM(): Seq=Null\n") if (! defined($seq) || length($seq) == 0);
    $seq = uc($seq);
    $seq =~ s/([CYSMBHV][GRSKBDV])/$drmM{$1}/g;
    # $seq =~ tr/CYSMBHV/ttkwkwd/;
    if ($retUC) { return(uc($seq)); }
    return($seq);
}
