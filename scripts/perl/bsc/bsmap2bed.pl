...
sub bsp2bed {
    ...
    die("\n[$func]: ERROR: Function call with missing paramaters (bspFile)\n")
	if (! defined($inpFile));
    die("\n[$func]: ERROR: Fasta File does not exist: '$inpFile'!\n") if (! -e $inpFile);

    my ($inpFH, $maBedFH, $umBedFH);
    my ($inpFileName, $inpFilePath, $inpFileSuffix, $isBin)=openFile(\$inpFH,$inpFile);
    mssg("[$func]: Loading file=$inpFile...\n", 2);

    my $maBedFile=$opts{'outDir'}."/bsmap.MA.bed";
    my $umBedFile=$opts{'outDir'}."/bsmap.UM.bed";
    open($maBedFH, ">$maBedFile") or die("Failed to open $maBedFile for writing: $!");
    open($umBedFH, ">$umBedFile") or die("Failed to open $umBedFile for writing: $!");

    # open($maBspFH, ">:$maBedFile", $bspFile) or die("Failed to open $maBedFile for writing: $!")
    # open($umBspFH, ">:$umBspFile", $bspFile) or die("Failed to open $umBedFile for writing: $!")

    my $loadAll=FALSE;
    my ($totCnt, $tenCnt, $curCnt);
    if ($loadAll) {
	my @lines=<$inpFH>;
	close($inpFH);
	chomp(@lines);
	($totCnt, $tenCnt, $curCnt) = tenCnt(scalar(@lines));
    } else {
	$totCnt=1000000;
	$tenCnt=int($totCnt/10);
	$curCnt=0;
    }
	...
	next if ($bspTag eq "NM");
	next if ($bspTag eq "OF");
	...
	my $bspRefSeq = $data[8];
	my $bspMisCnt = $data[9];
	my $bspMisStr = $data[10];

	my $prbLen = length($prbSeq);
	
	print {$maBedFH} join($TAB, $bspChr,$bspPos,($bspPos+$prbLen-1),$bspKey,$bspTag,$bspSrd,$prbSeq,$bspRefSeq,$bspGapCnt,$bspMisCnt,$bspMisStr)."\n"
	    if ($bspTag eq "MA");
	print {$umBedFH} join($TAB, $bspChr,$bspPos,($bspPos+$prbLen-1),$bspKey,$bspTag,$bspSrd,$prbSeq,$bspRefSeq,$bspGapCnt,$bspMisCnt,$bspMisStr)."\n"
	    if ($bspTag eq "UM");
	...
	if ($curCnt % $tenCnt == 0) {
	    runTimes();
	    mssg("[$func]: ".sprintf("%2d", 100*$curCnt/$totCnt)."\% ($curCnt/$totCnt). bspKey=$bspKey, RunTime=$runTimes[0]\n", 3);
	}
	$curCnt++;
    ...
    my $cnts_str=hashStr(\%cnts);

    mssg("[$func]: Current Count=$curCnt\n",2);
    mssg("[$func]: BSP Tags::\n",2);
    print STDERR $cnts_str;

    mssg("[$func]: Sorting/Indexing Beds::\n",2);
    sortBgzipIdx($maBedFile,FALSE);
    sortBgzipIdx($umBedFile,FALSE);
...
if (! defined($opts{'outDir'}) || ! defined($opts{'bspFile'}) || ! defined($opts{'mode'}) ) {
    print STDERR $usage;
...
if ($opts{'mode'} eq "bsp2bed") {
    bsp2bed($opts{'bspFile'});
} elsif ($opts{'mode'} eq "filter") {

} else {
    die("[ERROR]: mode=opts{'mode'} is not supported.\n\n");
}
...
sub sortBgzipIdx {
    ...
    $header="| grep -v $header" if (defined($header) && length($header)!=0);
