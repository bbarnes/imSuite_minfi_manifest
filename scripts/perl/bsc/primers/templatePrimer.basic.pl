...
##
my $verbose = TRUE;
my ($topDir, $outDir, $logDir, $bedDir, $datDir, $bamDir);
...
sub buildDirs {
    ...
    $outDir =~ s/\/+$//;
    system("rm -rf $outDir") if (defined($cleanAll) && -e $outDir);
    ...
    system("rm -f $datDir/* &> $datDir/cleanDat.log");

    $bamDir = "$bamDir/bam";
    make_path($bamDir) if (! -e $bamDir);
    system("rm -f $bamDir/* &> $bamDir/cleanBam.log");
}
...
sub loadBed {
    ...
    close(BED);
    mssg("[Bed]: Loaded " . scalar(keys %{$datRef}) . "\n\n", 2);
